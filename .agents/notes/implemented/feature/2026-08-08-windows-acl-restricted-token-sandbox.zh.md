...
Git Bash/MSYS2 与 Cygwin 不兼容受限模式，因为其 runtime 使用显式 DACL 创建按用户共享的 section 与信号 named pipe；这些 DACL 包含用户 SID，却没有 restricting SID。把用户 SID 加入 restricting list 会恢复对白名单外调用者自有文件的写入。runner 因此会在令牌建立前以保留的失败特征拒绝 `bash`/`sh`，并指示调用方使用 `sandbox_permissions=danger-full-access`；它不会弱化令牌，也不会静默改为不限权运行。Windows runner 套件固定这条快速失败指引。
