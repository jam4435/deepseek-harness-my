...
Plugin configuration and the Loader inventory each registered a top-level `settings.section`. They described the same Plugins domain but occupied two navigation rows, split search and configuration into unrelated pages, and gave the Settings shell no principled way to present them together. Combining their components directly would instead make one feature plugin import and own the other feature's data lifecycle.
...
The section owner contributes a `configurable` tab that declares the existing nested `settings.plugin.item` list. Configuration cards keep their namespace bindings, draft state, validation, and writes unchanged. `@deepseek-ai/dsh-client-ui-settings-plugin-inventory` contributes an `all` tab to `settings.plugins.tab`; its Host Loader observer, generated Remote namespace, and search semantics remain unchanged, while the DTO and panel later gained diagnostics and lifecycle controls ([decision](../feature/2026-08-15-plugin-inventory-lifecycle-controls.md)). Disabled inventory entries omit the redundant unmounted runtime state from summaries and details, while enabled entries expose their Cordis phase and runtime diagnostics.
...
Feature ownership remains explicit: `ui-settings-plugins` owns the Plugins page and editable cards, `ui-settings-plugin-inventory` owns the inventory view and its lifecycle controls, and the Host/RPC path changed only through the inventory gateway's own opt-in mutations. A new Plugins view can join by registering one `settings.plugins.tab` contribution.
