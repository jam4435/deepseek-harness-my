...
A preset revision is one composition while it has an Agent or cold-reader lease, not one composition per session. The roster mounts an exact immutable revision under a synthetic standing scope; each agent joins by binding its scope key to the mount's (`bindScopeParent(agentKey, standingKey)`). Two `dsh-scope` mechanisms carry everything: registration views walk the parent chain (`agent → preset → global`, nearest shadowing farthest), and scoped dispatch admits listeners tagged with an ancestor of the carrier key — upward only, so a sibling preset's listeners stay deaf. [Immutable preset revisions and request-content transforms](2026-08-19-immutable-preset-revisions-and-request-content.md) owns revision publication and request replay.

## Consequences

Standing mounts fix the class, not the instances: the registrations a reader needs exist while a lease holds the exact `id@revision`, and a cold reader can acquire that lease without starting an agent. What made it cheap
...
- **The standing map is keyed by immutable `id@revision`.** New sessions resolve the latest published revision; joined agents, child agents, and cold readers retain the referenced materialization. After the final lease releases, the Cordis scope and its private materialization are disposed while the immutable revision remains retained.
