...
一个 preset revision 在有 Agent 或冷读取 lease 时是一份组装，而不是每会话一份。roster 在合成常驻 scope 下挂载精确不可变 revision；每个 agent 通过把自己的 scope key 绑定到挂载的 key（`bindScopeParent(agentKey, standingKey)`）加入。两条 `dsh-scope` 机制承载了一切：注册视图沿父链解析（`agent → preset → global`，近者遮蔽远者），带作用域的分发对标签为载体键祖先的监听器放行——只向上，兄弟 preset 的监听器保持失聪。[不可变预设 revision 与 request-content 转换](2026-08-19-immutable-preset-revisions-and-request-content.md)负责 revision 发布和请求回放。

## 后果

常驻挂载修的是这一类问题而非其中的个例：读取方需要的注册在 lease 持有精确 `id@revision` 时存在，冷读取者无需启动 agent 即可取得该 lease。让它便宜的原因：
...
- **常驻映射以不可变 `id@revision` 为键。** 新会话解析最新发布 revision；已加入的 agent、子代理和冷读取者会保留所引用的物化副本。最后一个 lease 释放后，Cordis 作用域及其私有物化副本被销毁，而不可变 revision 会继续保留。
