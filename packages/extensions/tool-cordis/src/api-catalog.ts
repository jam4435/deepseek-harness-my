...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'async openEditor(id: string): Promise<{ readonly id: string readonly trust: \'system\' | \'user\' readonly composition: string readonly revision: string readonly name?: string readonly description?: string }>',
        description: 'Read the documents a browser editor needs together with their revision.',
        parameters: [{ name: 'id', description: 'preset id to open.' }],
      ...
      {
        signature: 'async create(id: string, name?: string): Promise<void>',
        description: 'Create an empty, editable preset.',
        parameters: [{ name: 'id', description: 'immutable directory id for the preset.' }, { name: 'name', description: 'initial display name.' }],
      },
      {
        signature: 'async save( id: string, expectedRevision: string, draft: { composition: string; name?: string; description?: string; files?: readonly PresetFileOperation[] }, ): Promise<string>',
        description: 'Persist an editor draft for a custom preset.',
        parameters: [{ name: 'id', description: 'preset id being changed.' }, { name: 'expectedRevision', description: 'version returned by {@link openEditor}.' }, { name: 'draft', description: 'metadata and complete composition text.' }],
        returns: 'the revision produced by the saved directory.',
      },
      {
        signature: 'async listFiles(id: string, directory: string = \'\', cursor: string = \'\', limit: number = 200): Promise<{ entries: PresetFileEntry[]; nextCursor?: string }>',
        description: 'List a safe, stable page of one preset\'s complete directory tree.',
        parameters: [{ name: 'id', description: 'preset id to inspect.' }, { name: 'directory', description: 'normalized directory prefix, or the root when empty.' }, { name: 'cursor', description: 'opaque continuation cursor from a prior page.' }, { name: 'limit', description: 'maximum number of entries in the page.' }],
        returns: 'file entries and an optional continuation cursor.',
      },
      {
        signature: 'async readFile(id: string, path: string): Promise<PresetFileContent>',
        description: 'Read one regular file as bounded UTF-8 or Base64 content.',
        parameters: [{ name: 'id', description: 'preset id to inspect.' }, { name: 'path', description: 'normalized POSIX path relative to the preset root.' }],
        returns: 'bounded text or binary content.',
      },
      {
        signature: 'async import( sourceId: string, id: string, json: string, preview: SillyTavernImportPreview, name?: string, ): Promise<void>',
        description: 'Import one validated SillyTavern document into a new custom preset.',
        parameters: [{ name: 'sourceId', description: 'Harness preset used as the base composition.' }, { name: 'id', description: 'new immutable preset id.' }, { name: 'json', description: 'original JSON upload.' }, { name: 'preview', description: 'preview produced by {@link inspectSillyTavernPreset}.' }, { name: 'name', description: 'optional display name.' }],
      },
      {
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    name: 'agent/response',
    mode: 'waterfall',
    signature: '\'agent/response\'(this: Scoped<Agent>, payload: { agent: Agent; turn: number; step: number; message: AssistantMessage; signal: AbortSignal }, next: () => Promise<AssistantMessage>): Promise<AssistantMessage>',
    summary: 'Rewrite the assembled assistant message before it is committed to the durable transcript.',
    description: 'Rewrite the assembled assistant message before it is committed to the durable transcript. Consumers must delegate when they do not own a transformation; the returned message is the one model history and replay will use on the next request.',
    parameters: [{ name: 'payload', description: '.signal - the current turn cancellation signal. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): listeners receive only this agent.' }],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'PresetFileContent',
    declaration: 'export type PresetFileContent = {\n    kind: \'text\';\n    path: string;\n    size: number;\n    content: string;\n} | {\n    kind: \'binary\';\n    path: string;\n    size: number;\n    base64: string;\n};',
  },
  {
    name: 'PresetFileEntry',
    declaration: 'export interface PresetFileEntry {\n    path: string;\n    type: \'directory\' | \'file\';\n    size: number;\n    executable: boolean;\n}',
  },
  {
    name: 'PresetOption',
  ...
  {
    name: 'SillyTavernImportIssue',
    declaration: 'export interface SillyTavernImportIssue {\n    readonly disposition: \'converted\' | \'approximated\' | \'preserved\' | \'rejected\';\n    readonly subject: string;\n    readonly message: string;\n}',
  },
  {
    name: 'SillyTavernImportKind',
    declaration: 'export type SillyTavernImportKind = \'chat-completion\' | \'regex\' | \'instruct\' | \'context\' | \'system-prompt\' | \'reasoning\' | \'text-completion\' | \'unknown\';',
  },
  {
    name: 'SillyTavernImportPreview',
    declaration: 'export interface SillyTavernImportPreview {\n    readonly kind: SillyTavernImportKind;\n    readonly prompts: readonly SillyTavernPromptEntry[];\n    readonly regexes: readonly SillyTavernRegexEntry[];\n    readonly sampling: {\n        readonly temperature?: number;\n        readonly maxTokens?: number;\n        readonly stop?: readonly string[];\n    };\n    readonly issues: readonly SillyTavernImportIssue[];\n}',
  },
  {
    name: 'SillyTavernPromptEntry',
    declaration: 'export interface SillyTavernPromptEntry {\n    readonly id: string;\n    readonly name: string;\n    readonly content: string;\n    readonly enabled: boolean;\n    readonly role: \'system\' | \'user\' | \'assistant\';\n    readonly position: \'relative\' | \'depth\';\n    readonly depth?: number;\n    readonly order: number;\n}',
  },
  {
    name: 'SillyTavernRegexEntry',
    declaration: 'export interface SillyTavernRegexEntry {\n    readonly id: string;\n    readonly name: string;\n    readonly find: string;\n    readonly replace: string;\n    readonly flags?: string;\n    readonly target?: \'request\' | \'response\';\n    readonly disabled: boolean;\n}',
  },
  {
    name: 'SkillCandidate',
  ...
  },
  {
    ...
    declaration: 'export type SurfaceEventType = \'user/message\' | \'prompt/message\' | \'assistant/message\' | \'tool/result\';',
  },
  {
    name: 'SurfaceOp',
