...
async function main(): Promise<number> {
  ...
  const executable = basename(parsed.command).toLowerCase()
  if (executable === 'bash' || executable === 'bash.exe' || executable === 'sh' || executable === 'sh.exe') {
    fail('Git Bash/MSYS2 and Cygwin cannot run under the Windows ACL WRITE_RESTRICTED token; retry with sandbox_permissions=danger-full-access')
  }
