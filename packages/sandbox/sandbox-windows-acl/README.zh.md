...
- **Git Bash/MSYS2 与 Cygwin 无法在受限模式下运行。** 它们的 runtime 使用显式 DACL 创建按用户共享的 section 与信号 named pipe；这些 DACL 授权用户 SID，却不包含本后端的任何 restricting SID。把用户 SID 加入 restricting list 会重新授权对白名单外调用者自有文件的写入，击穿隔离。runner 因此会在受限令牌建立前以保留的退出码 127 失败，并指示调用方通过显式 `sandbox_permissions=danger-full-access` 路径重试；它绝不会静默改为不限权运行。
