...
describe.skipIf(!isWin32 || !pwshAvailable())('windows-acl runner', () => {
  ...
  it('rejects Git Bash before restricted-token setup and directs the caller to explicit escalation', () => {
    const result = runRunner([
      '--workspace', writableDir, '--temp', isolatedTemp, '--mode', 'workspace-write',
      '--', 'bash', '-c', 'exit 99',
    ])
    expect(result.status).toBe(127)
    expect(result.stderr).toContain('windows-acl-run: Git Bash/MSYS2 and Cygwin cannot run under the Windows ACL WRITE_RESTRICTED token')
    expect(result.stderr).toContain('sandbox_permissions=danger-full-access')
  }, 15_000)
