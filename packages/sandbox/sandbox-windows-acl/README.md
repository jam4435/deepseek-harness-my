...
- **Git Bash/MSYS2 and Cygwin cannot run confined.** Their runtime creates per-user shared sections and signal named pipes with explicit DACLs that grant the user SID but none of this backend's restricting SIDs. Adding the user SID to the restricting list would authorize writes to caller-owned files outside the allowlist and break confinement. The runner therefore fails before restricted-token setup with its reserved exit 127 and directs callers to retry through the explicit `sandbox_permissions=danger-full-access` path; it never silently runs these shells unconfined.
