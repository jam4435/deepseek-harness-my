...
- **Git Bash/MSYS2 与 Cygwin 搭配 Windows ACL 后端时需要 `danger-full-access`**：它们按用户共享的 section 与信号 pipe 使用显式 DACL；若不向用户 SID 授予宽泛写入权，就无法满足 `WRITE_RESTRICTED`。后端会在令牌建立前以 `SANDBOX_UNAVAILABLE` 拒绝；部署允许时请用 `sandbox_permissions: danger-full-access` 重试。
