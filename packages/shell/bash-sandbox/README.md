...
- **Git Bash/MSYS2 and Cygwin require `danger-full-access` with the Windows ACL backend** — their per-user sections and signal pipes use explicit DACLs that cannot satisfy `WRITE_RESTRICTED` without granting the user SID broad write authority. The backend rejects them before token setup as `SANDBOX_UNAVAILABLE`; retry with `sandbox_permissions: danger-full-access` when the deployment permits it.
