...
| 字段 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `enableRunInBackground` | boolean | `true` | 公开 `run_in_background`；设为 `false` 会移除该参数，并在执行时拒绝强制后台调用。 |
| `description` | string | 生成值 | 完整的模型侧工具描述。部署方可以描述自己的 shell 环境，例如 Windows Git Bash 主机；省略时保留标准 bash 方言描述，空值或纯空白值会在加载时失败。 |
...
模型会看到生成的 [`bash` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-bash)，或部署方通过 `description` 配置的完整描述。仅当此生产方启用 `run_in_background` 时，该字段才会出现；仅当已挂载执行器声明支持沙箱时，`sandbox_permissions` 和 `justification` 才会出现。Agent 作用域的工具限制可以移除该 agent 的定义。

#### Token 影响

工具可见的每个请求都会产生固定 schema 开销；沙箱支持会增加升权字段及其条件说明段落。配置的 `description` 会替换生成文本，因此其 token 开销就是部署方写入的文本。
