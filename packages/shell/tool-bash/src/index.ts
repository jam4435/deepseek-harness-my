...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  if (configuredDescription !== undefined && configuredDescription.length === 0) {
    throw new Error('tool-bash: description must be non-empty')
  }
  ...
  ctx.tools.register(defineTool({
    name: 'bash',
    description: configuredDescription ?? bashDescription(backgroundEnabled, escalationModes),
    parameters: {
