...
async function setup(config: { description?: string; enableRunInBackground?: boolean } = {}) {
...
describe('bash tool', () => {
  ...
  it('uses a configured deployment description verbatim for the schema', async () => {
    const ctx = await setup({ description: 'Custom deployment bash description.' })
    expect(ctx.tools.schemas()[0]?.description).toBe('Custom deployment bash description.')
  })

  it('rejects an empty configured description at load (misconfiguration fails loud)', async () => {
    const ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(LocalSubprocessRuntime)
    ...
    await ctx.plugin(BashEnvPlugin)
    await ctx.plugin(LocalBashExecutor, { timeoutMs: 10_000, graceMs: 200 })
    await expect(ctx.plugin(ToolBash, { description: '   ' })).rejects.toThrow('tool-bash: description must be non-empty')
  })
