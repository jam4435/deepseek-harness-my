...
import type { AssistantMessage, LlmCallConfig, LlmFailure, ResolvedRetryPolicy } from '@deepseek-ai/dsh-llm'
...
/** The complete model-visible request content after preset runtime transforms. */
export interface RequestContent {
  /** Ordered request messages. These are independent from durable chat history. */
  readonly messages: readonly Message[]
  /** Rendered system instruction for this exact request, when non-empty. */
...
declare module '@deepseek-ai/cordis' {
  interface Events {
    ...
    */
    ...
    /**
     * Build the complete model-visible content for one request after the raw
     * entered messages have been committed. The default preserves the recorded
     * history and current input, plus the rendered system instruction. Consumers
     * may only transform this request copy; they must not rewrite history.
     * @param payload.agent - the agent making the request.
     * @param payload.history - durable messages preceding `input`.
     * @param payload.input - raw messages committed for this step.
     * @param payload.assembly - current system-prompt and tool assembly.
     ...
     * @mode waterfall
     */
    'agent/request-content'(this: Scoped<Agent>, payload: { agent: Agent; history: readonly Message[]; input: readonly UserMessage[]; assembly: PromptAssembly; turn: number; step: number; signal: AbortSignal }, next: () => Promise<RequestContent>): Promise<RequestContent>
    /**
