...
/** Whether two request batches carry the exact same durable message objects. */
...
export class ReactLoopAgent implements Agent {
    ...
    try {
      while (true) {
        ...
        try {
          ...
          const system = renderPrompt(decision.assembly)
          const content = await this.dispatch.waterfall(
            ...
            (): Promise<RequestContent> => Promise.resolve({
              messages: [...history, ...decision.messages],
              ...system === '' ? {} : { system },
            }),
          )
          signal.throwIfAborted()
          if (!sameMessages(content.messages, this.session.deriveMessages())) {
            this.session.append('request/messages', { turn, step, messages: content.messages })
          }
          // max-tokens is sticky: once any step hits the ceiling, later steps
          // that complete normally must not downgrade the turn outcome.
          const stepEnd = await this.step(decision.assembly, content)
  ...
  private async step(assembly: PromptAssembly, content: RequestContent): Promise<StepEndReason | null> {
    ...
    while (true) {
      const { request, preparedCall } = await this.buildRequest(
        turn, step, assembly.tools, content, signal,
      )
      ...
      const committedMessage = await this.dispatch.waterfall(
        'agent/response',
        { message, turn, step, signal },
      ...
      const toolCalls = committedMessage.content.filter(block => block.type === 'tool-call')
  ...
  private async buildRequest(
    ...
    const header = canonicalHeader({
      ...
      ...content.system === undefined ? {} : { system: content.system },
