...
import type { AgentPresetReference } from './agent-preset.ts'

export { AgentPresetId, AgentPresetRevision } from './agent-preset.ts'
export type { AgentPresetReference } from './agent-preset.ts'
...
export interface SessionEventMap {
  ...
  /** Prompt-program message with an explicit model role, appended after step/start. */
  'prompt/message': { turn: number; step: number; message: Message }
  ...
  /**
   * Exact message batch sent to the model when request-only composition differs
   * from ordinary derived history. This is log-only: surface/history projection
   * deliberately ignores it, while request replay can reconstruct the model
   * input for its owning turn and step.
   */
  'request/messages': { turn: number; step: number; messages: readonly Message[] }
  /**
 ...
 * `prompt/message`, `assistant/message`, `tool/result`).
