...
import { AgentPresetId, AgentPresetRevision, SESSION_FORMAT_VERSION, SessionId } from './types.ts'
...
export { canonicalHeader, foldRequestHeader, headerEquals, requestMessagesFor } from './request-header.ts'
...
function validateSessionHeader(id: SessionId, input: unknown): SessionHeader {
  ...
  if (record.agentPreset !== undefined) {
    if (record.agentPreset === null || typeof record.agentPreset !== 'object' || Array.isArray(record.agentPreset)) {
      throw new Error('session header agentPreset must be an exact preset reference')
    }
    const preset = record.agentPreset as Record<string, unknown>
    if (typeof preset.id !== 'string' || typeof preset.revision !== 'string') {
      throw new Error('session header agentPreset must carry string id and revision')
    }
    record.agentPreset = { id: AgentPresetId(preset.id), revision: AgentPresetRevision(preset.revision) }
  }
...
export function adoptSessionEvent<T extends SessionEvent>(event: T): T {
  ...
  switch (event.type) {
    ...
    case 'request/messages':
      for (const message of event.data.messages) deepFreeze(message)
      break
    default:
...
function assertCurrentLlmShape(event: Record<string, unknown>, index: number): void {
  ...
  if (event['type'] === 'request/messages') {
    ...
    if (!Number.isSafeInteger(turn) || (turn as number) < 1
      ...
      throw new Error(`seed request/messages at index ${index} has invalid position or messages`)
    }
    for (const [messageIndex, message] of messages.entries()) {
      assertRequestSnapshotMessage(message, `seed request/messages at index ${index}, message ${messageIndex}`)
    }
  }
  const type = event['type']
  if (type !== 'user/message' && type !== 'prompt/message' && type !== 'assistant/message'
...
function assertMessageEventShape(event: Record<string, unknown>, subject: string): void {
  ...
  const expectedRole = type === 'assistant/message' ? 'assistant' : type === 'user/message' ? 'user' : undefined
  if (expectedRole !== undefined && messageRecord['role'] !== expectedRole) {
...
/** Validate the role-neutral messages captured for exact request replay. */
function assertRequestSnapshotMessage(value: unknown, subject: string): void {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new Error(`${subject} is not a message`)
  }
  ...
  if (typeof message['id'] !== 'string' || message['id'] === ''
    ...
    throw new Error(`${subject} has invalid message fields`)
  }
  const source = message['source']
  if (typeof source !== 'object' || source === null || Array.isArray(source)
    || typeof (source as Record<string, unknown>)['kind'] !== 'string'
    || (source as Record<string, unknown>)['kind'] === '') {
    throw new Error(`${subject} has invalid message source`)
  }
}
