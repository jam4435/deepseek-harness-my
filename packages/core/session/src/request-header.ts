...
/**
 * Find a request-only message snapshot for one model step. Absent means the
 * step used ordinary derived history, so callers derive that history instead.
 * @param events - full session log or a step-bounded prefix.
 * @param turn - owning turn number.
 * @param step - owning step number.
 * @returns the exact model-visible messages, or undefined when no override was needed.
 */
export function requestMessagesFor(events: readonly SessionEvent[], turn: number, step: number): readonly Message[] | undefined {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index]!
    if (event.type === 'request/messages' && event.data.turn === turn && event.data.step === step) {
