...
import { AgentPresetId, AgentPresetRevision } from '@deepseek-ai/dsh-session'
import type { AgentPresetReference, Session, SessionEvent, SessionHeader, SessionId } from '@deepseek-ai/dsh-session'
...
function headerBindings(header: SessionHeader): (string | number | null)[] {
  return [
    ...
    header.agentPreset === undefined ? null : JSON.stringify(header.agentPreset),
  ]
}
...
function sameHeader(a: SessionHeader, b: SessionHeader): boolean {
  return a.version === b.version
    ...
    && a.agentPreset?.id === b.agentPreset?.id
    && a.agentPreset?.revision === b.agentPreset?.revision
}

function rowHeader(row: SessionHeaderRow): SessionHeader {
  const agentPreset = parseAgentPresetReference(row.agent_preset)
  return {
    version: row.version,
...
/** Decode the query index's serialized immutable preset reference. */
function parseAgentPresetReference(value: string | null): AgentPresetReference | undefined {
  if (value === null) return undefined
  try {
    const parsed: unknown = JSON.parse(value)
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) return undefined
    const reference = parsed as { id?: unknown; revision?: unknown }
    if (typeof reference.id !== 'string' || typeof reference.revision !== 'string') return undefined
    return { id: AgentPresetId(reference.id), revision: AgentPresetRevision(reference.revision) }
  } catch {
    // A derived index can contain a row written before exact preset references.
    // Treat the stale field as absent; the source session remains authoritative.
    return undefined
  }
}
