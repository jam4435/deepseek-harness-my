...
import SessionStore, { AgentPresetId, AgentPresetRevision, SESSION_FORMAT_VERSION, SessionId } from '@deepseek-ai/dsh-session'
...
describe('SQLite session search', () => {
  ...
  it('searches two-character Unicode61 tokens in live-only sessions', async () => {
    ...
    const session = ctx.sessions.create(SessionId('live'), {
      ...
      meta: {
        cwd: '/work', createdAt: 10, seedLength: 1, delegationDepth: 2,
