...
describe('agent-presets invariants', () => {
  it('reclaims a standing composition after its final agent lease releases', async () => {
    ...
    // Immutable materializations are retained by the revision store; a live
    // Cordis generation remains only while an agent or cold reader leases it.
    await handle.dispose()
    expect(livePresetMounts().map(mount => mount.presetId)).not.toContain('standard')

    // A later agent gets a fresh materialization of the same exact revision.
  ...
  it('admits a joined agent, a scopeless read, and a standing-key read', async () => {
    ...
    const standing = await ctx.agentPresets.acquireStandingLease('standard')
