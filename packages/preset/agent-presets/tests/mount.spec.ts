...
import SessionStore, { SessionId, type AgentPresetReference } from '@deepseek-ai/dsh-session'
...
async function agentOn(ctx: Context, id: string, presetId?: string | AgentPresetReference): Promise<Agent> {
...
describe('composing a child agent from its parent', () => {
  ...
  it('reports the preset id the child joined, for the durable header', async () => {
    ...
    expect(ctx.agentPresets.composedPreset(parent.ctx)?.id).toBe('minimal')
    expect(ctx.agentPresets.composedPreset(child.ctx)?.id).toBe('minimal')
  })
...
describe('composing from a broken preset', () => {
  ...
  it('refuses the mount up front with the discovery-reported reason', async () => {
    ...
    await expect(agentOn(scoped, 'sess-broken-2', 'damaged')).rejects.toThrow(/failed to parse config/)
    expect(livePresetMounts().filter(mount => mount.presetId === 'damaged')).toHaveLength(0)
  })

  it('refuses the immutable generation a cold reader would mount by', async () => {
    const scoped = await rosterWith('rows: not-a-list\n')

    await expect(scoped.agentPresets.acquireStandingLease('damaged'))
      .rejects.toThrow(/failed to validate config file/)
  })
...
describe('replacing a composition', () => {
  it('publishes a committed preset selection for remote consumers', async () => {
    ...
    const selected: Array<[SessionId, AgentPresetReference]> = []
    ctx.on('agent-preset/selected', (sessionId, agentPreset) => {
      selected.push([sessionId, agentPreset])
    })

    const selectedPreset = (await ctx.agentPresets.latestReference('minimal')).reference
    agent.session.append('agent-preset/selected', { agentPreset: selectedPreset })

    expect(selected).toEqual([[SessionId('sess-selected'), selectedPreset]])
  })
...
describe('editing a composition file', () => {
  ...
  it('mounts independently published revisions of one preset', async () => {
    const { scoped, path } = await editable('guarded-refresh')
    const firstRevision = (await scoped.agentPresets.latestReference('guarded-refresh')).reference
    const first = await agentOn(scoped, 'sess-guarded-refresh-seed', firstRevision)

    await writeFile(path, rowFor('afterwards'))
    const secondRevision = (await scoped.agentPresets.latestReference('guarded-refresh')).reference
    const second = await agentOn(scoped, 'sess-guarded-refresh-later', secondRevision)
  ...
  it('hands a host reader the standing key without starting an agent', async () => {
    ...
    const first = await scoped.agentPresets.acquireStandingLease('cold-read')

    // The mount exists for the reader; no agent, session, or turn started.
    expect(first.key).toMatchObject({ agentPreset: expect.stringMatching(/^cold-read@/) })
    ...
    const second = await scoped.agentPresets.acquireStandingLease('cold-read')
    expect(second.key).toBe(first.key)
    first.release()
    second.release()
  })

  it('serves a published revision after its editable working copy is removed', async () => {
    const { scoped, path } = await editable('unstampable')
    const reference = (await scoped.agentPresets.latestReference('unstampable')).reference
    await rm(path)

    const lease = await scoped.agentPresets.acquireStandingLease(reference)

    expect(lease.key).toMatchObject({ agentPreset: expect.stringMatching(/^unstampable@/) })
    expect(livePresetMounts().filter(mount => mount.presetId === 'unstampable')).toHaveLength(1)
    lease.release()
  })

  it('keeps an agent on its mounted revision when its working file is removed', async () => {
