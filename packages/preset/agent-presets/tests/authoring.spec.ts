...
import AgentPresets, {
  COMPOSITION_FILE, copyComposition, inspectSillyTavernPreset, METADATA_FILE,
...
describe('visual preset authoring', () => {
  it('creates an empty preset and saves metadata plus its composition under a revision lock', async () => {
    await ctx.agentPresets.create('blank', 'Blank canvas')
    const opened = await ctx.agentPresets.openEditor('blank')

    expect(opened).toMatchObject({ trust: 'user', name: 'Blank canvas', composition: '[]\n' })
    const revision = await ctx.agentPresets.save('blank', opened.revision, {
      name: 'Search only', description: 'Uses the web search tool.', composition: VALID,
    })

    expect(revision).not.toBe(opened.revision)
    expect(await ctx.agentPresets.openEditor('blank')).toMatchObject({
      name: 'Search only', description: 'Uses the web search tool.', composition: VALID, revision,
    })
  })

  it('refuses a stale browser draft instead of overwriting a newer preset', async () => {
    await ctx.agentPresets.copy('standard', 'mine')
    const first = await ctx.agentPresets.openEditor('mine')
    const second = await ctx.agentPresets.openEditor('mine')
    await ctx.agentPresets.save('mine', first.revision, { composition: VALID })

    await expect(ctx.agentPresets.save('mine', second.revision, { composition: '{}\n' }))
      .rejects.toThrow(/changed while it was being edited/)
  })

  it('lists and atomically edits safe file-tree entries', async () => {
    await ctx.agentPresets.create('files')
    const opened = await ctx.agentPresets.openEditor('files')
    const before = await ctx.agentPresets.listFiles('files')
    expect(before.entries.some(entry => entry.path === COMPOSITION_FILE)).toBe(true)

    const revision = await ctx.agentPresets.save('files', opened.revision, {
      composition: opened.composition,
      files: [
        { kind: 'mkdir', path: 'docs' },
        { kind: 'write-text', path: 'docs/readme.txt', content: 'hello' },
      ],
    })
    expect(revision).not.toBe(opened.revision)
    expect((await ctx.agentPresets.readFile('files', 'docs/readme.txt'))).toMatchObject({ kind: 'text', content: 'hello' })
    await expect(ctx.agentPresets.readFile('files', '../outside.txt')).rejects.toThrow(/relative POSIX|normalized/)
    await expect(ctx.agentPresets.save('files', revision, {
      composition: opened.composition,
      files: [{ kind: 'rename', path: COMPOSITION_FILE, target: 'renamed.yml' }],
    })).rejects.toThrow(/managed/i)
  })

  it('never writes a shipped preset through the editor', async () => {
    const opened = await ctx.agentPresets.openEditor('standard')

    await expect(ctx.agentPresets.save('standard', opened.revision, { composition: '{}\n' }))
      .rejects.toThrow(/ships with the deployment/)
  })

  it('creates an imported preset from a Harness base and retains source plus report files', async () => {
    const json = JSON.stringify({ prompts: [{ identifier: 'system', content: 'Be concise.' }] })
    const preview = inspectSillyTavernPreset(json)

    await ctx.agentPresets.import('standard', 'tavern', json, preview, 'Tavern import')

    expect(await readFile(join(userRoot, 'tavern', COMPOSITION_FILE), 'utf8'))
      .toContain('@deepseek-ai/dsh-prompt-program')
    expect(await readFile(join(userRoot, 'tavern', '.dsh', 'sillytavern', 'source.json'), 'utf8')).toBe(json)
    expect(JSON.parse(await readFile(join(userRoot, 'tavern', '.dsh', 'sillytavern', 'report.json'), 'utf8')))
      .toMatchObject({ kind: 'chat-completion', prompts: [{ id: 'system', content: 'Be concise.' }] })
  })
})
