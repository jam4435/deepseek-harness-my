...
import { AgentPresetId, AgentPresetRevision, SessionId } from '@deepseek-ai/dsh-session'
import type { AgentPresetReference, SessionEvent, SessionHeader } from '@deepseek-ai/dsh-session'
import { resolveSessionPreset } from '../src/session.ts'

/** A header carrying the creation-time preset, if any. */
function reference(id: string): AgentPresetReference {
  return { id: AgentPresetId(id), revision: AgentPresetRevision(id.padEnd(64, '0')) }
}

function header(agentPreset?: AgentPresetReference): SessionHeader {
  return {
    version: 0,
    id: SessionId('s'),
    createdAt: 1,
...
function selected(agentPreset: AgentPresetReference, seq: number): SessionEvent {
...
describe('resolving which preset a session ran', () => {
  it('reads the creation-time value when nothing was switched', () => {
    expect(resolveSessionPreset({ header: header(reference('standard')), events: [] })).toEqual(reference('standard'))
  })
  ...
  it('prefers a logged switch over the header', () => {
    ...
    expect(resolveSessionPreset({ header: header(reference('standard')), events: [selected(reference('minimal'), 0)] }))
      .toEqual(reference('minimal'))
  })

  it('takes the last switch when a session was moved twice', () => {
    expect(resolveSessionPreset({
      header: header(reference('standard')),
      events: [selected(reference('minimal'), 0), selected(reference('cordis'), 1)],
    })).toEqual(reference('cordis'))
  })

  it('finds a switch behind later events', () => {
    const later = { type: 'turn/end', seq: 2, time: 2, data: { turn: 1 } } as SessionEvent

    expect(resolveSessionPreset({ header: header(), events: [selected(reference('minimal'), 0), later] }))
      .toEqual(reference('minimal'))
  })
