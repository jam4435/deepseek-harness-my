...
Authoring lands under the first `user` root. `copy()` creates a whole-directory copy of an existing preset — composition, metadata, skill directories, and assets — while `create()` starts an empty `agent.cordis.yml`. `openEditor()` returns editable metadata, exact composition text, a safe file-tree page, and a directory revision; `save()` accepts metadata, composition, and staged file operations atomically, and refuses a stale revision instead of replacing an edit made through another browser or a native editor. Shipped presets remain readable but are never writable through these methods. File paths are normalized POSIX relatives, reserved files are managed only by their dedicated editors, and version publication enforces 64 MiB save, 256 MiB version, and 10,000-entry limits. `copy()` refuses three things before anything lands:
...
The revision is a deterministic SHA-256 digest of every regular file and directory below the preset root, including relative path and owner-execute bit. Symbolic links and non-regular entries are rejected while hashing, so the browser editor cannot use a preset operation to inspect or write outside its directory. Text reads are capped at 1 MiB and one attachment at 16 MiB; binary data crosses the API only as bounded Base64.

### SillyTavern import

The Host exposes `agentPreset.importPreview` and `agentPreset.importCreate` for one JSON upload at a time. Preview parses data only: it never evaluates Handlebars macros, JavaScript, extension payloads, or regular expressions. Chat Completion prompts are converted with their order, role, depth, and enabled state; regex scripts are listed with their find, replacement, and disabled state; provider-specific sampling is retained only when it maps to the provider-neutral `temperature`, `maxTokens`, or `stop` fields. Instruct, Context, System Prompt, Reasoning, Text Completion, Kobold, Novel, and unknown documents remain in the original source file and are reported as preserved or rejected when no executable mapping exists.

Import always creates a new custom preset by copying a selected Harness base. The original JSON and deterministic compatibility report are written under `.dsh/sillytavern/`; an existing custom preset is never merged or overwritten. The import dialog lets the author toggle converted prompt and regex entries before creation, and the report distinguishes `converted`, `approximated`, `preserved`, and `rejected` results.

Imported prompt and regex data are written into dedicated prompt-program and safe-regex adapter rows in the copied base. Prompt messages are logged with their role-bearing `prompt/message` event, while regex replacements run in worker-isolated request/response phases with bounded time and memory-related text sizes.
