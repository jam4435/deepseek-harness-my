...
import {
  copyComposition, createComposition, deleteComposition, importComposition, listPresetFiles, readPresetFile, readComposition, saveComposition,
  PRESET_ENTRY_LIMIT, PRESET_VERSION_LIMIT,
  type PresetFileContent, type PresetFileEntry, type PresetFileOperation,
} from './authoring.ts'
import { mountPreset, serviceForAgent } from './mount.ts'
import { PresetExistsError, PresetRevisionConflictError } from './authoring.ts'
...
import { disposeMaterializedPreset, materializePresetRevision } from './revisions.ts'
import type { AgentPresetEditorDocument } from './editor-types.ts'
...
export {
  copyComposition, createComposition, deleteComposition, importComposition, listPresetFiles, readPresetFile, InvalidPresetIdError, PresetExistsError,
  PresetNotWritableError, PresetRevisionConflictError, readComposition, revisionForDirectory, saveComposition, updateComposition, writableRoot,
} from './authoring.ts'
export type { PresetFileContent, PresetFileEntry, PresetFileOperation } from './authoring.ts'
export { resolveSessionPreset, type PresetBearingSession } from './session.ts'
export {
  defaultPresetRevisionRoot, disposeMaterializedPreset, materializePresetRevision,
  publishPresetRevision, revisionForPresetDirectory,
} from './revisions.ts'
export type { PresetRevisionLimits } from './revisions.ts'
export { inspectSillyTavernPreset } from './sillytavern.ts'
...
export type { AgentPresetEditorDocument, SillyTavernImportInspection } from './editor-types.ts'
...
export class AgentPresets extends Service {
  ...
  static Config = z.object({
    ...
    revisionRoot: z.string().default(dshHomePath('.agent-preset-revisions')),
    mountRoot: z.string().default(dshHomePath('.agent-preset-mounts')),
    versionBytes: z.natural().min(1).default(PRESET_VERSION_LIMIT),
    revisionEntries: z.natural().min(1).default(PRESET_ENTRY_LIMIT),
  }) as z<Config>
  ...
  /** Fully resolved limits and directories used after schema defaults are applied. */
  ...
  constructor(ctx: Context, config: Config) {
    ...
    this.config = {
      ...config,
      revisionRoot: config.revisionRoot ?? dshHomePath('.agent-preset-revisions'),
      mountRoot: config.mountRoot ?? dshHomePath('.agent-preset-mounts'),
      versionBytes: config.versionBytes ?? PRESET_VERSION_LIMIT,
      revisionEntries: config.revisionEntries ?? PRESET_ENTRY_LIMIT,
    }
    this.resolvedRoots = this.config.includeUserRoot
      ? [...this.config.roots, { path: dshHomePath(USER_PRESET_DIR), trust: 'user' }]
      : [...this.config.roots]
  ...
  /** Limits used while publishing an immutable working-tree revision. */
  private get revisionLimits(): { readonly versionBytes: number; readonly entries: number } {
    return { versionBytes: this.config.versionBytes, entries: this.config.revisionEntries }
  }

  /**
   * Resolve the current editable directory and publish its exact immutable identity.
   * @param id - preset id, or the configured default when omitted.
   * @returns roster metadata and the revision that new sessions must record.
   */
  async latestReference(id?: string): Promise<{ readonly preset: AgentPreset; readonly reference: AgentPresetReference }> {
    const preset = await this.resolve(id)
    return {
      preset,
      reference: await revisionForPresetDirectory(preset.id, dirname(preset.path), this.revisionLimits, this.config.revisionRoot),
    }
  }
   ...
   */
  /**
   * Standing mounts by exact revision, single-flight so two agents racing the
  ...
  /** Exact standing generation retained by each live agent scope. */
  private readonly joined = new WeakMap<ScopeKey, StandingMount>()

  /** Agent scopes whose disposal has been wired to release their mount lease. */
  private readonly agentLeaseCleanup = new WeakSet<ScopeKey>()
   ...
   */
  async mount(agentCtx: Context, requested?: string | AgentPresetReference): Promise<AgentPresetReference> {
    ...
    const resolved = await this.resolveMountableRevision(requested)
    const standing = await this.ensureStanding(resolved)
    ...
    this.retainAgent(agentCtx, standing)
   ...
   */
  composeFrom(agentCtx: Context, parentCtx: Context): AgentPresetReference | undefined {
    const agentKey = scopeOf(agentCtx)
    if (agentKey === undefined) {
      throw new Error('agent-presets: refusing to compose an unscoped context; the scope key is what joins an agent to its preset')
    }
    const parentKey = scopeOf(parentCtx)
    const standing = parentKey === undefined ? undefined : this.joined.get(parentKey)
    if (standing === undefined) return undefined
    this.bindings.set(agentKey, bindScopeParent(agentKey, standing.key))
    this.retainAgent(agentCtx, standing)
    return standing.reference
  }
   ...
   */
  composedPreset(agentCtx: Context): AgentPresetReference | undefined {
    const agentKey = scopeOf(agentCtx)
    return agentKey === undefined ? undefined : this.joined.get(agentKey)?.reference
  }
   ...
   * @throws when no configured root supplies that id.
   */
  async read(requested: string | AgentPresetReference): Promise<string> {
    const resolved = await this.resolvePresetRevision(requested)
    try {
      return await readComposition(resolved.preset)
    } finally {
  ...
  /**
   * Read the documents a browser editor needs together with their revision.
   * @param id - preset id to open.
   * @returns current composition, metadata, trust, and optimistic-lock token.
   */
  async openEditor(requested: string | AgentPresetReference): Promise<AgentPresetEditorDocument> {
    const id = typeof requested === 'string' ? requested : requested.id as string
    const { preset, reference } = await this.latestReference(id)
    return {
      agentPreset: reference,
      trust: preset.trust,
   ...
   */
  async copy(from: string | AgentPresetReference, id: string, name?: string): Promise<AgentPresetReference> {
    const source = await this.resolvePresetRevision(from)
    ...
    try {
      await copyComposition(this.resolvedRoots, source.preset, id, name)
    } finally {
      await source.dispose()
    }
    try {
      await this.validateWorkingPreset(id)
    } catch (error) {
    ...
    return (await this.latestReference(id)).reference
  }

  /**
   * Create an empty, editable preset.
   * @param id - immutable directory id for the preset.
   * @param name - initial display name.
   */
  async create(id: string, name?: string): Promise<AgentPresetReference> {
    if ((await this.list()).some(preset => preset.id === id)) throw new PresetExistsError(id)
    await createComposition(this.resolvedRoots, id, name)
    try {
      await this.validateWorkingPreset(id)
    } catch (error) {
      await deleteComposition(this.resolvedRoots, await this.resolve(id))
  ...
  /**
   * Persist an editor draft for a custom preset.
   * @param id - preset id being changed.
   * @param expectedRevision - version returned by {@link openEditor}.
   * @param draft - metadata and complete composition text.
   * @returns the revision produced by the saved directory.
   */
  async save(
    requested: AgentPresetReference,
    ...
    const preset = await this.resolve(id)
    const revision = await saveComposition(
      this.resolvedRoots,
      preset,
      requested.revision as string,
      draft,
      async directory => await this.validatePresetDirectory(id, directory),
    )
    return { id: requested.id, revision: revision as AgentPresetReference['revision'] }
  }

  /**
   * List a safe, stable page of one preset's complete directory tree.
   * @param id - preset id to inspect.
   * @param directory - normalized directory prefix, or the root when empty.
   * @param cursor - opaque continuation cursor from a prior page.
   * @param limit - maximum number of entries in the page.
   * @returns file entries and an optional continuation cursor.
   */
  async listFiles(requested: string | AgentPresetReference, directory: string = '', cursor: string = '', limit: number = 200): Promise<{ entries: PresetFileEntry[]; nextCursor?: string }> {
    const resolved = await this.resolvePresetRevision(requested)
    try {
      return await listPresetFiles(resolved.preset, directory, cursor, limit)
    } finally {
      await resolved.dispose()
    }
  }
  ...
  /**
   * Read one regular file as bounded UTF-8 or Base64 content.
   * @param id - preset id to inspect.
   * @param path - normalized POSIX path relative to the preset root.
   * @returns bounded text or binary content.
   */
  async readFile(requested: string | AgentPresetReference, path: string): Promise<PresetFileContent> {
    const resolved = await this.resolvePresetRevision(requested)
    try {
      return await readPresetFile(resolved.preset, path)
    } finally {
      await resolved.dispose()
    }
  }
  ...
  /**
   * Import one validated SillyTavern document into a new custom preset.
   * @param sourceId - Harness preset used as the base composition.
   * @param id - new immutable preset id.
   * @param json - original JSON upload.
   * @param preview - preview produced by {@link inspectSillyTavernPreset}.
   * @param name - optional display name.
   */
  async import(
    sourceId: string | AgentPresetReference,
    id: string,
    json: string,
    preview: SillyTavernImportPreview,
    name?: string,
  ): Promise<AgentPresetReference> {
    const source = await this.resolvePresetRevision(sourceId)
    if ((await this.list()).some(preset => preset.id === id)) throw new PresetExistsError(id)
    try {
      await importComposition(this.resolvedRoots, source.preset, id, name, json, preview)
    } finally {
      await source.dispose()
    }
    try {
   ...
   */
  async remove(requested: string | AgentPresetReference): Promise<void> {
    const id = typeof requested === 'string' ? requested : requested.id as string
    if (typeof requested !== 'string') await this.assertLatest(requested)
    await deleteComposition(this.resolvedRoots, await this.resolve(id))
   ...
   * @throws when the preset is unknown or its composition is unusable.
   */
  async recompose(agentCtx: Context, requested: string | AgentPresetReference): Promise<AgentPresetReference> {
    const agentKey = scopeOf(agentCtx)
    if (agentKey === undefined) {
  ...
  /**
   * Acquire one cold-reader lease for a standing preset generation.
   *
   * The lease keeps the materialized revision and its Cordis scope alive until
   * a transcript projection or catalog query has finished. It never starts an
   * agent or session.
   * @param requested - exact revision, preset id, or the configured default.
   * @returns the registry scope and an idempotent release action.
   * @throws when the preset is unknown or its composition is unusable.
   */
  async acquireStandingLease(requested?: string | AgentPresetReference): Promise<StandingMountLease> {
    const resolved = await this.resolveMountableRevision(requested)
    const standing = await this.ensureStanding(resolved)
    standing.leases += 1
    let released = false
    return {
      ...
      release: () => {
        if (released) return
        released = true
        this.releaseStanding(standing)
      },
    }
  }

  /** Materialized immutable revision plus its one-shot cleanup action. */
  private async resolvePresetRevision(requested: string | AgentPresetReference | undefined): Promise<ResolvedPresetRevision> {
    if (typeof requested === 'string' || requested === undefined) {
      const current = await this.latestReference(requested)
      // Id-based internal callers resolve the current working copy. This keeps
      // development-only relative rows loadable while public/session callers
      // always supply an immutable reference and take the materialized path
      // below. A newly published revision still keys the standing generation.
      return { reference: current.reference, preset: current.preset, dispose: async () => {} }
    }
    const reference = requested
    let current: AgentPreset | undefined
    try {
      current = await this.resolve(reference.id as string)
    } catch (error) {
      if (!(error instanceof UnknownPresetError)) throw error
    }
    await mkdir(this.config.mountRoot, { recursive: true, mode: 0o700 })
    const directory = join(this.config.mountRoot, `${reference.id as string}-${reference.revision as string}-${randomUUID()}`)
    await materializePresetRevision(this.config.revisionRoot, reference, directory)
    return {
      reference,
      preset: {
        id: reference.id as string,
        trust: current?.trust ?? 'user',
        path: join(directory, 'agent.cordis.yml'),
        ...current?.name === undefined ? {} : { name: current.name },
        ...current?.description === undefined ? {} : { description: current.description },
      },
      dispose: async () => await disposeMaterializedPreset(directory),
    }
  }

  /** Resolve an immutable revision that is about to mount into a standing scope. */
  private async resolveMountableRevision(requested: string | AgentPresetReference | undefined): Promise<ResolvedPresetRevision> {
    const resolved = await this.resolvePresetRevision(requested)
    try {
      await stat(resolved.preset.path)
      return resolved
    } catch (error) {
      await resolved.dispose()
      throw new PresetMountError(resolved.preset.id, `composition file is unreadable: ${resolved.preset.path}`, { cause: error })
    }
  }

  /** Load and release a candidate directory in an isolated scope before publishing it. */
  private async validatePresetDirectory(id: string, directory: string): Promise<void> {
    const scope = createScope(this.selfCtx, { presetValidation: randomUUID() })
    try {
  ...
  /** Validate a newly authored working copy and remove no running generation. */
  private async validateWorkingPreset(id: string): Promise<void> {
    const preset = await this.resolve(id)
    await this.validatePresetDirectory(id, dirname(preset.path))
  }

  /** Reject a stale mutating reference without replacing another editor's work. */
  private async assertLatest(reference: AgentPresetReference): Promise<void> {
    const current = (await this.latestReference(reference.id as string)).reference
    if (current.revision !== reference.revision) {
      throw new PresetRevisionConflictError(reference.id as string, reference.revision as string, current.revision as string)
    }
  }

  /** Attach one live agent to a standing generation, releasing a former one after a successful rebind. */
  private retainAgent(agentCtx: Context, standing: StandingMount): void {
    const agentKey = scopeOf(agentCtx)
    if (agentKey === undefined) return
    const former = this.joined.get(agentKey)
    if (former === standing) return
    this.joined.set(agentKey, standing)
    standing.leases += 1
    if (!this.agentLeaseCleanup.has(agentKey)) {
      this.agentLeaseCleanup.add(agentKey)
      agentCtx.effect(() => () => this.releaseAgent(agentCtx), 'agent-presets: agent generation lease')
    }
    if (former !== undefined) this.releaseStanding(former)
  }

  /** Release the generation held by an agent that left the registry. */
  private releaseAgent(agentCtx: Context): void {
    const agentKey = scopeOf(agentCtx)
    if (agentKey === undefined) return
    const standing = this.joined.get(agentKey)
    if (standing === undefined) return
    this.joined.delete(agentKey)
    this.releaseStanding(standing)
  }

  /** Dispose an unreferenced materialized generation without touching its retained blobs. */
  private releaseStanding(standing: StandingMount): void {
    if (standing.leases === 0) return
    standing.leases -= 1
    if (standing.leases > 0) return
    const key = revisionKey(standing.reference)
    if (this.standing.get(key) === undefined) return
    this.standing.delete(key)
    void standing.scope.dispose().finally(standing.dispose)
  }

  /** Resolve (or create, single-flight) the standing mount of one exact revision. */
  private async ensureStanding(resolved: ResolvedPresetRevision): Promise<StandingMount> {
    const key = revisionKey(resolved.reference)
    const pending = this.standing.get(key)
    if (pending !== undefined) {
      await resolved.dispose()
      return await pending
    }
    const created = (async (): Promise<StandingMount> => {
      const scopeKey: ScopeKey = { agentPreset: key }
      const scope = createScope(this.selfCtx, scopeKey)
      try {
        await mountPreset(scope.ctx, resolved.preset, resolved.reference)
        return { key: scopeKey, scope, reference: resolved.reference, dispose: resolved.dispose, leases: 0 }
      } catch (error) {
        this.standing.delete(key)
        await scope.dispose()
        await resolved.dispose()
    ...
    this.standing.set(key, created)
    return created
  }
}

/** One exact revision plus a private directory that must be removed once unused. */
interface ResolvedPresetRevision {
  readonly reference: AgentPresetReference
  readonly preset: AgentPreset
  dispose(): Promise<void>
}

/** Stable map key for one exact immutable preset revision. */
function revisionKey(reference: AgentPresetReference): string {
  return `${reference.id as string}@${reference.revision as string}`
}
...
interface StandingMount {
  ...
  /** Immutable revision the scope was materialized from. */
  readonly reference: AgentPresetReference
  /** Removes the private materialization after this generation is released. */
  dispose(): Promise<void>
  /** Number of live agents parented to this generation. */
  leases: number
}

/** A cold reader's temporary hold on an immutable standing composition. */
export interface StandingMountLease {
  /** Scope key passed to registry and presenter lookups. */
  readonly key: ScopeKey
  /** Release the hold after the read is complete. Safe to call more than once. */
  release(): void
}
