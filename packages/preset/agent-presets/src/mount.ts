...
export interface PresetMount {
  ...
  /** Exact immutable revision the mount's files came from. */
 ...
 */
export async function mountPreset(agentCtx: Context, preset: AgentPreset, reference?: AgentPresetReference): Promise<void> {
  ...
  try {
    ...
    mounts.add({ presetId: preset.id, ...reference === undefined ? {} : { reference }, fiber, key: scopeOf(agentCtx) })
  } catch (error) {
    try {
