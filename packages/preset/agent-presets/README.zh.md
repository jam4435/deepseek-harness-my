...
创作落在首个 `user` 根目录之下。`copy()` 创建一个既有 preset 的整目录副本——组装、元数据、skill 目录和附带资产；`create()` 则从空的 `agent.cordis.yml` 开始。`openEditor()` 返回可编辑元数据、原样组装文本、安全文件树页面和目录版本；`save()` 原子接收元数据、组装和排队的文件操作，若另一浏览器或原生编辑器已修改目录则拒绝过期版本，不会覆盖。随附 preset 仍可读取，但这些方法绝不写入它们。文件路径会规范化为 POSIX 相对路径，保留文件只能由对应编辑器管理；版本发布限制保存 64 MiB、版本 256 MiB 和 10,000 个目录项。`copy()` 在任何内容落盘之前拒绝三种情况：
...
版本是 preset 根目录下每个常规文件和目录的确定性 SHA-256 摘要，其中包括相对路径和属主执行位。计算哈希时会拒绝符号链接和非常规条目，因此浏览器编辑器不能借 preset 操作读取或写入目录以外的位置。文本读取限制为 1 MiB，单个附件限制为 16 MiB；二进制数据只以受限 Base64 通过 API。

### 酒馆导入

Host 提供 `agentPreset.importPreview` 和 `agentPreset.importCreate`，每次接收一个 JSON 文件。预览阶段只解析数据，不执行 Handlebars 宏、JavaScript、扩展字段或正则表达式。Chat Completion 的提示词会转换顺序、角色、深度和启用状态；正则脚本会列出查找表达式、替换文本和禁用状态；提供商专属采样字段只有能映射到通用 `temperature`、`maxTokens` 或 `stop` 时才会转换。Instruct、Context、System Prompt、Reasoning、Text Completion、Kobold、Novel 以及无法识别的文档会保留原始文件；没有可执行映射时会在报告中标为保留或拒绝。

导入始终以选定的 Harness 底座创建新的自定义预设。原始 JSON 和确定性的兼容报告写入 `.dsh/sillytavern/`；不会合并或覆盖已有自定义预设。导入弹窗允许在创建前切换已转换的提示词和正则条目，报告使用 `converted`、`approximated`、`preserved`、`rejected` 四类结果。

导入的提示词和正则数据会写入复制底座中的提示词程序与安全正则适配器行。提示词消息通过带角色的 `prompt/message` 事件记录；正则替换在 Worker 隔离的请求/回复阶段执行，并限制时间与文本大小。
