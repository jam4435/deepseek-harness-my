...
import {
  Button, IconCloseFill14, IconPersonalizationOutline16, IconRefreshOutline16,
...
export function WorkspaceBrowser({
  ...
  const restartWeb = (): void => {
    if (restarting || !window.confirm('重启 Web 服务？当前页面会短暂断开，并由后台启动器自动恢复。')) return
    setRestarting(true)
    void fetch('/api2/plugin-manager/restartWeb', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: '{}',
    }).then((response) => {
      if (!response.ok) throw new Error('restart request rejected')
  ...
  return (
    <div className={clsx(css.root, !wide && css.rail)}>
      <div className={css.sectionHeader}>
        ...
        {wide && (
          <Tooltip label={restarting ? '正在重启 Web 服务' : '重启 Web 服务'} side="bottom" delayMs={500}>
            <button
              type="button"
              className={css.iconButton}
              aria-label={restarting ? '正在重启 Web 服务' : '重启 Web 服务'}
              disabled={restarting}
              onClick={restartWeb}
            >
