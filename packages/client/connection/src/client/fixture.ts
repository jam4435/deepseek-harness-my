...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, {
    ...
    ['standard', { trust: 'system', content: "- id: tool-bash\n  name: '@deepseek-ai/dsh-tool-bash'\n", revision: 'fixture-standard' }],
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@deepseek-ai/dsh-tool-web-search'\n", revision: 'fixture-minimal' }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@deepseek-ai/dsh-tool-read'\n", revision: 'fixture-my-agent' }],
  ])
  const presetReference = (id: string, preset: { revision: string }): AgentPresetReference => ({
    id: id as AgentPresetReference['id'],
    revision: preset.revision as AgentPresetReference['revision'],
  })
    ...
    agentPresets: {
      ...
      list: request => ok(request, {
        presets: [...fixturePresets].map(([id, preset]) => ({
          id,
          reference: presetReference(id, preset),
          trust: preset.trust,
      ...
      select: (request) => {
        fixtureDefaultPreset = request.payload.agentPreset.id as string
        return ok(request, { agentPreset: request.payload.agentPreset })
      },
      read: (request) => {
        const { agentPreset } = request.payload
        const id = agentPreset.id as string
        const preset = fixturePresets.get(id)
        if (preset === undefined) {
          return err(request, {
            code: 'agent-preset-not-found',
            message: `unknown agent preset "${id}"`,
            details: { agentPreset: id, available: [...fixturePresets.keys()] },
          })
        }
        return ok(request, {
          agentPreset,
      ...
      create: (request) => {
        const { agentPreset, name, source } = request.payload
        if (fixturePresets.has(agentPreset)) {
          return err(request, {
            code: 'agent-preset-invalid', message: `agent preset "${agentPreset}" already exists`,
            details: { agentPreset, reason: 'already exists' },
          })
        }
        const base = source === undefined ? undefined : fixturePresets.get(source.id as string)
        fixturePresets.set(agentPreset, {
          trust: 'user', content: base?.content ?? '[]\n', revision: `fixture-${++fixturePresetRevision}`,
          ...name === undefined ? {} : { name },
        })
        const created = fixturePresets.get(agentPreset)
        /* v8 ignore next -- the row was inserted immediately above. */
        if (created === undefined) throw new Error(`fixture lost preset ${agentPreset}`)
        return ok(request, { agentPreset: presetReference(agentPreset, created) })
      },
      importPreview: request => ok(request, {
        sourceHash: '0'.repeat(64),
        preview: {
          kind: 'unknown' as const, prompts: [], regexes: [], sampling: {},
          issues: [{ disposition: 'preserved' as const, subject: 'fixture', message: 'Fixture import preview.' }],
        },
      }),
      importCreate: (request) => {
        const { agentPreset, source, name } = request.payload
        const sourceId = source.id as string
        const base = fixturePresets.get(sourceId)
        if (base === undefined) return err(request, {
          code: 'agent-preset-not-found', message: `unknown base preset "${sourceId}"`,
          details: { agentPreset: sourceId, available: [...fixturePresets.keys()] },
        })
        if (fixturePresets.has(agentPreset)) return err(request, {
          code: 'agent-preset-invalid', message: `agent preset "${agentPreset}" already exists`,
          details: { agentPreset, reason: 'already exists' },
        })
        fixturePresets.set(agentPreset, { trust: 'user', content: base.content, revision: `fixture-${++fixturePresetRevision}`, ...name === undefined ? {} : { name } })
        const created = fixturePresets.get(agentPreset)
        /* v8 ignore next -- the row was inserted immediately above. */
        if (created === undefined) throw new Error(`fixture lost preset ${agentPreset}`)
        return ok(request, { agentPreset: presetReference(agentPreset, created) })
      ...
      editorOpen: (request) => {
        ...
        if (preset === undefined) {
          return err(request, {
            code: 'agent-preset-not-found', message: `unknown agent preset "${id}"`,
            details: { agentPreset: id, available: [...fixturePresets.keys()] },
          })
        }
        return ok(request, {
          agentPreset: presetReference(id, preset), trust: preset.trust, composition: preset.content,
          ...preset.name === undefined ? {} : { name: preset.name },
          ...preset.description === undefined ? {} : { description: preset.description },
        })
      },
      editorSave: (request) => {
        const { agentPreset, composition, name, description } = request.payload
        const id = agentPreset.id as string
        const preset = fixturePresets.get(id)
        if (preset === undefined || preset.trust !== 'user') {
          return err(request, {
            code: 'agent-preset-read-only', message: `agent preset "${id}" cannot be edited`,
            details: { agentPreset: id, reason: 'not a custom preset' },
          })
        }
        if (preset.revision !== (agentPreset.revision as string)) {
          return err(request, {
            code: 'internal', message: `agent preset "${id}" changed; reload before saving`,
            details: {},
          })
        }
        preset.content = composition
        if (name === undefined) delete preset.name
        else preset.name = name
        if (description === undefined) delete preset.description
        else preset.description = description
        preset.revision = `fixture-${++fixturePresetRevision}`
        return ok(request, { agentPreset: presetReference(id, preset) })
      },
      editorListFiles: request => ok(request, { entries: [] }),
      editorReadFile: request => err(request, {
        code: 'internal', message: `fixture has no file "${request.payload.path}"`, details: {},
      }),
      editorTestRegex: request => ok(request, {
        output: request.payload.text,
        captures: [],
        elapsedMs: 0,
      }),
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'agentPreset.importPreview': return this.api.agentPresets.importPreview(request)
      case 'agentPreset.importCreate': return this.api.agentPresets.importCreate(request)
      case 'agentPreset.editorOpen': return this.api.agentPresets.editorOpen(request)
      case 'agentPreset.editorSave': return this.api.agentPresets.editorSave(request)
      case 'agentPreset.editorListFiles': return this.api.agentPresets.editorListFiles(request)
      case 'agentPreset.editorReadFile': return this.api.agentPresets.editorReadFile(request)
      case 'agentPreset.editorTestRegex': return this.api.agentPresets.editorTestRegex(request)
