...
export class FakeApiClient implements IApiClient {
  ...
  readonly agentPresets: IApiClient['agentPresets'] = {
    ...
    importPreview: (payload: unknown) => this.record('agentPreset.importPreview', payload, Promise.resolve(ok({
      kind: 'unknown' as const, prompts: [], regexes: [], sampling: {}, issues: [],
    }))),
    importCreate: (payload: { agentPreset: string }) =>
      this.record('agentPreset.importCreate', payload, Promise.resolve(ok({ agentPreset: payload.agentPreset }))),
    editorOpen: (payload: { agentPreset: string }) =>
      this.record('agentPreset.editorOpen', payload, Promise.resolve(ok({
        agentPreset: payload.agentPreset, trust: 'user' as const, composition: '{}\\n', revision: 'fake-revision',
      }))),
    editorSave: (payload: unknown) =>
      this.record('agentPreset.editorSave', payload, Promise.resolve(ok({ revision: 'fake-revision' }))),
    editorListFiles: (payload: unknown) =>
      this.record('agentPreset.editorListFiles', payload, Promise.resolve(ok({ entries: [] }))),
    editorReadFile: (payload: unknown) =>
      this.record('agentPreset.editorReadFile', payload, Promise.resolve(ok({ kind: 'text' as const, path: 'fixture.txt', size: 0, content: '' }))),
