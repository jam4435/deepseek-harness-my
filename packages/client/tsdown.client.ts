 ...
 */
export const INLINE_SAFE = /^@deepseek-ai\/dsh-(host-apiproxy|session|llm|tools|brand|api-remotes\/client-safe)(\/|$)/
...
const YAML_BROWSER_ENTRY = resolvePath(dirname(nodeRequire.resolve('yaml/package.json')), 'browser/index.js')
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    plugins: [{
      // yaml declares a browser export, but Rolldown's Node-first conditional
      // resolution selects its Node build while compiling client factories.
      // Resolve its documented browser entry directly so the emitted factory
      // does not retain Node's `process` dependency.
      name: 'dsh-yaml-browser-entry',
      resolveId(source: string) {
        return source === 'yaml' ? YAML_BROWSER_ENTRY : null
      },
    }, {
