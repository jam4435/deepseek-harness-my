...
export class AgentPresetSeatController {
   ...
   */
  private fallback: AgentPresetReference | undefined

  /** Set while a pick is waiting for a session; cleared once applied. */
  private staged: AgentPresetReference | undefined
  ...
  constructor(
     ...
     */
    private readonly onApplied?: (sessionId: string, agentPreset: AgentPresetReference) => void,
  ) {}
  ...
  async load(): Promise<void> {
    try {
      ...
      this.fallback = presets.find(preset => preset.isDefault)?.reference ?? presets[0]?.reference
      this.set({
        ...
        current: (this.staged ?? this.currentSession()?.agentPreset ?? this.fallback)?.id as string | undefined ?? '',
        error: null,
      })
    } catch (error) {
  ...
  async select(id: string): Promise<void> {
    ...
    const reference = this.store.getSnapshot().options.find(option => option.id === id)?.reference
   ...
   */
  stage(reference: AgentPresetReference, introduce = false): void {
    this.staged = reference
    this.set({ current: reference.id as string, error: null, introduce })
  }

  /** Stage one currently listed preset by id for callers outside the chip. */
  stageId(id: string, introduce = false): void {
    const reference = this.store.getSnapshot().options.find(option => option.id === id)?.reference
    if (reference !== undefined) this.stage(reference, introduce)
  }
  ...
  async apply(): Promise<void> {
    ...
    if (!session.blank || (session.agentPreset?.id === staged.id && session.agentPreset.revision === staged.revision)) {
    ...
    try {
      ...
      this.staged = undefined
      if (!response.result.ok) {
        this.set({ busy: false, error: response.result.error.message, current: this.fallback?.id as string | undefined ?? '' })
        return
      }
      // Consumed: the next new session opens on the deployment default again.
      this.set({ busy: false, current: response.result.value.agentPreset.id as string })
      this.onApplied?.(session.id, response.result.value.agentPreset)
    } catch (error) {
      this.staged = undefined
      this.set({ busy: false, error: messageOf(error), current: this.fallback?.id as string | undefined ?? '' })
    }
  }
}
