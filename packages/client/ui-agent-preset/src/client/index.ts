...
export function apply(ctx: ClientContext): void {
  ...
  const sectionInjected = (): AgentPresetSectionInjected => ({
    ...
    openEditor: (id: string) => section.openEditor(id),
    closeEditor: () => { section.closeEditor() },
    patchEditor: (patch) => { section.patchEditor(patch) },
    toggleCompositionRow: (path) => { section.toggleCompositionRow(path) },
    moveCompositionRow: (path, toIndex, targetParent) => { section.moveCompositionRow(path, toIndex, targetParent) },
    addCompositionRow: (row, parent) => { section.addCompositionRow(row, parent) },
    deleteCompositionRow: (path) => { section.deleteCompositionRow(path) },
    selectFile: path => section.selectFile(path),
    patchFileText: (path, content) => { section.patchFileText(path, content) },
    uploadFile: (path, file) => section.uploadFile(path, file),
    downloadFile: path => section.downloadFile(path),
    createFile: (path, directory) => { section.createFile(path, directory) },
    renameFile: (from, to) => { section.renameFile(from, to) },
    deleteFile: (path) => { section.deleteFile(path) },
    testRegex: (entry, text) => section.testRegex(entry, text),
    saveEditor: () => section.saveEditor(),
    beginCopy: (from: string) => { section.beginCopy(from) },
    beginBlank: () => { section.beginBlank() },
    ...
    beginImport: (source?: string) => { section.beginImport(source) },
    previewImport: (fileName: string, content: string) => section.previewImport(fileName, content),
    cancelImport: () => { section.cancelImport() },
    patchImport: (patch) => { section.patchImport(patch) },
    toggleImportPrompt: (id: string) => { section.toggleImportPrompt(id) },
    toggleImportRegex: (id: string) => { section.toggleImportRegex(id) },
