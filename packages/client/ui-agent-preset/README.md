...
A fourth surface, its own settings page (`settings.section` id `agent-presets`, ordered after Models): the roster as cards, an explicit creation dialog, and the Preset studio.

The dialog creates either an empty preset or a complete copy. The Preset studio opens every composition, makes shipped rows read-only, and lets a custom row edit metadata through forms, toggle plugin rows, add/delete rows, reorder siblings with keyboard-safe up/down controls, and keep the complete `agent.cordis.yml` source as an advanced fallback. Its opening revision accompanies `editorSave`; a concurrent native or browser edit is rejected and remains visible as an error instead of becoming a last-write-wins replacement. The immutable directory id stays visible but cannot be edited. The file-tree panel lists directories and regular files, edits bounded UTF-8 text, uploads/downloads binary data, and stages rename/move/delete operations for the same atomic save; `preset.yml` and `agent.cordis.yml` remain reserved. Where the host has no desktop opener (`hasDocument: false` on the roster; remote and container deployments), the action answers the directory as text on the row instead of offering a button that would spawn into nothing.
...
The same section can import one SillyTavern JSON file. The Host previews the file without executing macros or scripts, chooses a Harness base for the new custom preset, and returns a compatibility report. Converted Chat Completion prompts and regex scripts are shown as cards with independent enabled/disabled switches before creation. The original JSON and report are retained in the new preset so unsupported fields remain inspectable.
...
`agentPreset.read`, `copy`, `create`, `importPreview`, `importCreate`, `editorOpen`, `editorSave`, `editorListFiles`, `editorReadFile`, `openDocument`, and `remove` are loopback-pinned ([`dsh-client-connection`](../connection/README.md)): a composition names the plugins a session runs, so reading one is reconnaissance, and the rest manage the roster and drive the host desktop. `agentPreset.list` is not — it carries ids, trust, and the two path-free capability flags, and a LAN client's picker needs it.
...
- **Binary files are represented as bounded Base64** — the browser offers download rather than an in-page binary editor; text files use the bounded textarea editor.
