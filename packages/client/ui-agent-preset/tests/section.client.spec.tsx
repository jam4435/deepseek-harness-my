...
import type { AgentPresetSectionState, CopyDraft, ImportDraft } from '../src/client/section-store.ts'
...
function renderSection(
  ...
  const actions = {
    ...
    beginImport: vi.fn(),
    previewImport: vi.fn(() => Promise.resolve()),
    cancelImport: vi.fn(),
    patchImport: vi.fn(),
    toggleImportPrompt: vi.fn(),
    toggleImportRegex: vi.fn(),
    confirmImport: vi.fn(() => Promise.resolve()),
...
describe('the preset list', () => {
  ...
  it('shows the custom workspace even before the first import', () => {
    renderSection({ rows: [{ id: 'standard', trust: 'system', isDefault: true }] })

    expect(screen.getByRole('heading', { name: en.customGroup })).toBeTruthy()
    expect(screen.getByRole('button', { name: en.import })).toBeTruthy()
  })
...
describe('the SillyTavern import dialog', () => {
  const draft: ImportDraft = {
    source: 'standard', id: 'tavern', name: 'Tavern', fileName: 'preset.json', content: '{}', saving: false, error: null,
    preview: {
      kind: 'chat-completion',
      prompts: [{ id: 'system', name: 'System', content: 'Be concise.', enabled: true, role: 'system', position: 'relative', order: 0 }],
      regexes: [{ id: 'r1', name: 'Trim', find: 'x', replace: 'y', disabled: false }], sampling: {}, issues: [],
    },
  }

  it('shows converted entries with independent switches', () => {
    const actions = renderSection({ importDraft: draft })
    const dialog = screen.getByRole('dialog')

    expect(within(dialog).getByText(en.promptEntries)).toBeTruthy()
    expect(within(dialog).getByText(en.regexEntries)).toBeTruthy()
    const checkboxes = within(dialog).getAllByRole('checkbox')
    fireEvent.click(checkboxes[0])
    fireEvent.click(checkboxes[1])

    expect(actions.toggleImportPrompt).toHaveBeenCalledWith('system')
    expect(actions.toggleImportRegex).toHaveBeenCalledWith('r1')
  })
})
