...
interface FakePreset { trust: 'system' | 'user'; content: string; name?: string; description?: string; revision?: string }
...
function fakeApi(
  ...
  let revision = 0
  const tokenFor = (preset: FakePreset): string => preset.revision ?? 'revision-0'
  ...
  return {
    agentPresets: {
      ...
      create: (payload: { agentPreset: string; name?: string }) => {
      ...
      importPreview: (payload: { content: string }) => {
        record('importPreview', payload)
        return ok({
          kind: 'chat-completion', prompts: [{
            id: 'system', name: 'System', content: 'hello', enabled: true,
            role: 'system', position: 'relative', order: 0,
          }], regexes: [], sampling: {}, issues: [],
        })
      },
      importCreate: (payload: { source: string; agentPreset: string; name?: string }) => {
        record('importCreate', payload)
        const source = presets.get(payload.source)
        if (source === undefined) return fail(`unknown preset ${payload.source}`)
        presets.set(payload.agentPreset, {
          trust: 'user', content: source.content,
          ...payload.name === undefined ? {} : { name: payload.name },
        })
        return ok({ agentPreset: payload.agentPreset })
      },
      editorOpen: (payload: { agentPreset: string }) => {
        record('editorOpen', payload)
        const preset = presets.get(payload.agentPreset)
        if (preset === undefined) return fail(`unknown preset ${payload.agentPreset}`)
        return ok({
          agentPreset: payload.agentPreset, trust: preset.trust, composition: preset.content, revision: tokenFor(preset),
          ...preset.name === undefined ? {} : { name: preset.name },
          ...preset.description === undefined ? {} : { description: preset.description },
        })
      },
      editorSave: (payload: {
        agentPreset: string; expectedRevision: string; composition: string; name?: string; description?: string
      }) => {
        record('editorSave', payload)
        const preset = presets.get(payload.agentPreset)
        if (preset === undefined || preset.trust !== 'user') return fail('preset is read-only')
        if (payload.expectedRevision !== tokenFor(preset)) return fail('preset changed; reload before saving')
        preset.content = payload.composition
        if (payload.name === undefined) delete preset.name
        else preset.name = payload.name
        if (payload.description === undefined) delete preset.description
        else preset.description = payload.description
        preset.revision = `revision-${++revision}`
        return ok({ revision: preset.revision })
      },
...
describe('the visual preset studio', () => {
  it('opens a custom preset, saves its metadata and composition, then refreshes the roster', async () => {
    const { controller, calls, presets } = harness()
    await controller.load()

    await controller.openEditor('mine')
    controller.patchEditor({ name: 'Search mode', description: 'Only search.', composition: '{}\n' })
    await controller.saveEditor()

    expect(calls.map(call => call.method)).toContain('editorOpen')
    expect(calls.map(call => call.method)).toContain('editorSave')
    expect(presets.get('mine')).toMatchObject({ name: 'Search mode', description: 'Only search.', content: '{}\n' })
    expect(controller.store.getSnapshot().editor?.error).toBeNull()
  })

  it('keeps a conflict on the open draft rather than replacing a newer revision', async () => {
    const { controller, presets } = harness()
    await controller.openEditor('mine')
    const preset = presets.get('mine')
    if (preset === undefined) throw new Error('fixture preset missing')
    preset.revision = 'changed-elsewhere'

    await controller.saveEditor()

    expect(controller.store.getSnapshot().editor?.error).toContain('reload before saving')
  })

  it('creates a blank preset through the existing creation dialog', async () => {
    const { controller, calls, presets } = harness()
    await controller.load()
    controller.beginBlank()
    controller.setCopyId('blank')
    controller.setCopyName('Blank preset')

    await controller.confirmCopy()

    expect(calls.find(call => call.method === 'create')?.payload).toEqual({ agentPreset: 'blank', name: 'Blank preset' })
    expect(presets.get('blank')).toMatchObject({ trust: 'user', content: '[]\n' })
  })

  it('previews a SillyTavern file and creates a new preset from the selected base', async () => {
    const { controller, calls, presets } = harness()
    await controller.load()
    controller.beginImport('standard')
    controller.patchImport({ id: 'tavern', name: 'Tavern import' })
    await controller.previewImport('preset.json', '{"prompts":[]}')

    expect(controller.store.getSnapshot().importDraft?.preview).toMatchObject({ kind: 'chat-completion' })
    await controller.confirmImport()

    expect(calls.map(call => call.method)).toContain('importPreview')
    expect(calls.map(call => call.method)).toContain('importCreate')
    expect(presets.get('tavern')).toMatchObject({ trust: 'user', name: 'Tavern import', content: '- id: tool-bash\n' })
    expect(controller.store.getSnapshot().importDraft).toBeNull()
  })
})
