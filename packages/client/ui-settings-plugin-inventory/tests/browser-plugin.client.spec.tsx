...
const EMPTY = { mutationsEnabled: true, entries: [] }
type RemoteResult =
...
async function bench() {
  ...
  const list = vi.fn<() => Promise<RemoteResult>>()
    .mockResolvedValue({ ok: true, value: EMPTY })
  const mutate = vi.fn<() => Promise<RemoteResult>>()
    .mockResolvedValue({ ok: true, value: EMPTY })
  ...
  return { ctx, slots: ctx.get('slots') as SlotRegistry, locale, list, mutate }
}
...
describe('ui-settings-plugin-inventory browser plugin', () => {
  ...
  it('registers a localized tab without reading the Remote eagerly', async () => {
    ...
    await expect(injected.mutate('disable', 'entry-1')).resolves.toEqual(EMPTY)
    expect(b.mutate).toHaveBeenCalledWith('entry-1')
    b.mutate.mockResolvedValueOnce({ ok: false, error: { code: 'REMOTE_ERROR', message: 'unavailable' } })
    await expect(injected.mutate('restart', 'entry-1')).rejects.toThrow(
      'pluginInventory.restart failed: REMOTE_ERROR: unavailable',
    )
