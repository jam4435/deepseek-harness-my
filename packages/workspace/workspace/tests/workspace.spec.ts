...
describe('header-validated membership projection', () => {
  it('refreshes persisted header membership without changing the stored workspace account', async () => {
    const owned = await makeDir('refresh-persisted-header-membership')
    const id = WorkspaceId('owned')
    const pool = storedPool(
      [[id, record(owned, ['recovered'])]],
      { initialized: true, workspaceIds: [id] },
    )
    const result = await harness({ pool, sessions: [] })
    expect(result.registry.list()[0]!.sessionIds).toEqual([])

    result.setSessions([header('recovered', owned, 1)])
    await result.registry.refreshSessionHeaders()

    expect(result.registry.list()[0]!.sessionIds).toEqual(['recovered'])
    expect(storedRecord(pool, id).sessionIds).toEqual(['recovered'])
  })
