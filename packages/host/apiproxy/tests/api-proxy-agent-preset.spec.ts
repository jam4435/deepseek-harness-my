...
function roster(ids: readonly string[], userIds: readonly string[] = []): unknown {
  ...
  return {
    ...
    create: (_id: string, _name?: string) => Promise.resolve(),
    import: (_source: string, _id: string, _json: string, _preview: unknown, _name?: string) => Promise.resolve(),
    openEditor: (id: string) => Promise.resolve({
      id, trust: trustOf(id), composition: `- id: ${id}\n`, revision: 'revision-1',
    }),
    save: (_id: string, _expected: string, _draft: unknown) => Promise.resolve('revision-2'),
...
describe('authoring over the wire', () => {
  ...
  it('previews a SillyTavern document and accepts author-selected item switches', async () => {
    const { api } = await harness(['standard'])
    const content = JSON.stringify({ prompts: [{ identifier: 'system', content: 'Be concise.' }] })
    const previewResponse = await api.agentPresets.importPreview(request({ content }))
    expect(previewResponse.result.ok).toBe(true)
    if (!previewResponse.result.ok) throw new Error('unreachable')

    const preview = {
      ...previewResponse.result.value,
      prompts: previewResponse.result.value.prompts.map(prompt => ({ ...prompt, enabled: false })),
    }
    const created = await api.agentPresets.importCreate(request({
      source: 'standard', agentPreset: 'tavern', content, preview,
    }))

    expect(created.result).toMatchObject({ ok: true, value: { agentPreset: 'tavern' } })
  })

  it('opens and saves an editor draft through the revision API', async () => {
    const { api } = await harness(['standard'], undefined, { userIds: ['mine'] })
    const opened = await api.agentPresets.editorOpen(request({ agentPreset: 'mine' }))
    expect(opened.result).toMatchObject({ ok: true, value: { revision: 'revision-1', trust: 'user' } })
    const saved = await api.agentPresets.editorSave(request({
      agentPreset: 'mine', expectedRevision: 'revision-1', composition: '[]\n',
    }))
    expect(saved.result).toMatchObject({ ok: true, value: { revision: 'revision-2' } })
  })
