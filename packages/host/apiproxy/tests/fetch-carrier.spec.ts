  ...
  return {
    ...
    agentPresets: {
      ...
      create(request: RpcRequest<{ agentPreset: string }>) {
        return Promise.resolve({ rpcId: request.rpcId, result: { ok: true as const, value: { agentPreset: request.payload.agentPreset } } })
      },
      importPreview(request: RpcRequest<{ content: string }>) {
        return Promise.resolve({ rpcId: request.rpcId, result: { ok: true as const, value: { kind: 'unknown' as const, prompts: [], regexes: [], sampling: {}, issues: [] } } })
      },
      importCreate(request: RpcRequest<{ agentPreset: string }>) {
        return Promise.resolve({ rpcId: request.rpcId, result: { ok: true as const, value: { agentPreset: request.payload.agentPreset } } })
      },
      editorOpen(request: RpcRequest<{ agentPreset: string }>) {
        return Promise.resolve({
          rpcId: request.rpcId,
          result: { ok: true as const, value: { agentPreset: request.payload.agentPreset, trust: 'user' as const, composition: '', revision: 'test' } },
        })
      },
      editorSave(request: RpcRequest<{ agentPreset: string }>) {
        return Promise.resolve({ rpcId: request.rpcId, result: { ok: true as const, value: { revision: 'test-next' } } })
      },
