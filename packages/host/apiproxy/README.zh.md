...
创作界面还提供 `agentPreset.create`、`editorOpen`、`editorSave`、`editorListFiles`、`editorReadFile`，以及两阶段的酒馆导入 `importPreview`/`importCreate`。`create` 写入顶层空组装；编辑器返回元数据、完整源码和目录版本，并拒绝陈旧保存。导入预览接受一个 JSON 对象或独立正则数组，不执行其中内容，转换通用参数和 Chat Completion 提示词条目，并报告不支持的数据。导入创建始终复制选定的 Harness 底座，把原文件和报告保存到 `.dsh/sillytavern/`，并为转换出的提示词/正则条目追加运行时适配器行，并接受每条目的启用/禁用选择，但不会把它们当作可执行脚本。这些方法固定在环回地址，并限制上传为 16 MiB；提示词／正则运行时适配器仍由组装单独负责。
