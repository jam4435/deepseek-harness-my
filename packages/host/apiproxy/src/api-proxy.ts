...
import type { AgentPresetReference, JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@deepseek-ai/dsh-session'
...
import {
  inspectSillyTavernPreset, InvalidPresetIdError, PresetExistsError, PresetMountError,
...
import { testRegex } from '@deepseek-ai/dsh-regex-program'
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function assertPresetUnchanged(
    sessionId: SessionId,
    requested: AgentPresetReference | undefined,
    existing: AgentPresetReference | undefined,
  ): void {
    if (requested === undefined || (existing !== undefined
      && requested.id === existing.id && requested.revision === existing.revision)) return
    throw new AgentPresetConflict(sessionId, requested.id as string, existing?.id as string | undefined)
  }
   ...
   */
  async function composeAgent(requested: AgentPresetReference | undefined): Promise<{
    agentPreset?: AgentPresetReference
    ...
    const reference = requested ?? (await presets.latestReference()).reference
    return {
      agentPreset: reference,
      setup: async (agentCtx: Context) => {
        installSelection(agentCtx)
        await presets.mount(agentCtx, reference)
      },
    }
  }
   ...
   * @returns a temporary scope lease, or the global layer when no preset applies.
   */
  async function presenterScopeLeaseFor(
    sessionId: SessionId,
    session: PresetBearingSession,
  ): Promise<{ scope?: ScopeKey; release(): void }> {
    const live = ctx.get('agents')?.get(sessionId)
    if (live !== undefined) return { scope: live, release: () => {} }
    const presets = ctx.get('agentPresets')
    if (presets === undefined) return { release: () => {} }
    try {
      ...
      const lease = await presets.acquireStandingLease(resolveSessionPreset(session))
   ...
   */
  let legacyPresetHeaderUpgrade: Promise<void> | undefined

  /** Upgrade old floating preset ids before the persistence backend lists headers. */
  function upgradeLegacyPresetHeaders(): Promise<void> {
    legacyPresetHeaderUpgrade ??= (async () => {
      const persistence = ctx.get('sessionPersistence')
      const presets = ctx.get('agentPresets')
      if (persistence === undefined || presets === undefined) return
      const migrated = await persistence.upgradeLegacyAgentPresetHeaders(async (id) => {
        try {
          return (await presets.latestReference(id)).reference
        } catch {
          // A deleted preset cannot be truthfully reconstructed. Keep its
          // original log untouched so an operator can restore that preset first.
          return undefined
        }
      })
      // WorkspaceRegistry starts before this API endpoint. Its first header
      // index skipped legacy rows, so refresh it as soon as those rows become
      // readable instead of presenting every recovered session as Ungrouped.
      if (migrated > 0) await ctx.workspaceRegistry.refreshSessionHeaders()
    })()
    return legacyPresetHeaderUpgrade
  }

  async function listVisibleSessionSummaries(signal?: AbortSignal): Promise<SessionSummary[]> {
    signal?.throwIfAborted()
    await upgradeLegacyPresetHeaders()
    signal?.throwIfAborted()
  ...
  return {
    sessions: {
      ...
      async history(request) {
        ...
        try {
          ...
          const presenter = await presenterScopeLeaseFor(sessionId, sourceSession(source))
          try {
            const cut = historyCutOf(source, beforeSeq === undefined)
            const page = historyPage(ctx, cut.events, beforeSeq, maxMessages, presenter.scope)
            return ok(request, {
              events: page.events,
              hasMore: page.hasMore,
    ...
    agentPresets: {
      ...
      async list(request) {
        ...
        return ok(request, {
          presets: await Promise.all((await presets.list()).map(async preset => ({
            id: preset.id,
            reference: (await presets.latestReference(preset.id)).reference,
            trust: preset.trust,
      ...
      async select(request) {
        ...
        if (presets === undefined) {
          return err(request, {
            code: 'agent-preset-not-found',
            message: 'this deployment composes no agent presets',
            details: { agentPreset: agentPreset.id as string, available: [] },
          })
        }
        const found = await agentFor(sessionId)
        if ('error' in found) return err(request, found.error)
        const { agent } = found
        const swap = async (): Promise<RpcResponse<{ agentPreset: AgentPresetReference }>> => {
          ...
          if (!sessionBlank(agent.session)) {
            return err(request, {
              ...
              details: { sessionId, agentPreset: agentPreset.id as string },
            })
          }
          try {
            ...
            return ok(request, { agentPreset: preset })
          } catch (error: unknown) {
            const refused = presetFailure(request, error)
            if (refused !== undefined) return refused
            return err(request, {
              code: 'internal',
              message: `failed to select agent preset "${agentPreset.id as string}": ${String(error)}`,
              details: {},
            })
          }
        }
      ...
      async read(request) {
        ...
        const presets = ctx.get('agentPresets')
        if (presets === undefined) return err(request, noRoster(agentPreset.id as string))
        try {
          const preset = await presets.resolve(agentPreset.id as string)
          return ok(request, {
            agentPreset,
            trust: preset.trust,
            content: await presets.read(agentPreset),
            ...preset.name === undefined ? {} : { name: preset.name },
            ...preset.description === undefined ? {} : { description: preset.description },
          })
        } catch (error: unknown) {
          return err(request, presetError(agentPreset.id as string, error))
        }
      },

      async create(request) {
        const { agentPreset, source, name } = request.payload
        const presets = ctx.get('agentPresets')
        if (presets === undefined) return err(request, noRoster(agentPreset))
        try {
          const created = source === undefined
            ? await presets.create(agentPreset, name)
            : await presets.copy(source, agentPreset, name)
          return ok(request, { agentPreset: created })
        } catch (error: unknown) {
      ...
      async importPreview(request) {
        try {
          return ok(request, {
            sourceHash: createHash('sha256').update(request.payload.content).digest('hex'),
            preview: inspectSillyTavernPreset(request.payload.content),
          })
        } catch (error: unknown) {
          return err(request, presetError('SillyTavern import', error))
        }
      },

      async importCreate(request) {
        const { source, agentPreset, content, expectedHash, name } = request.payload
        const presets = ctx.get('agentPresets')
        if (presets === undefined) return err(request, noRoster(agentPreset))
        try {
          // Re-parse the uploaded bytes at the commit boundary. The preview is
          // informational and cannot be forged into a different file result.
          const actual = inspectSillyTavernPreset(content)
          const hash = createHash('sha256').update(content).digest('hex')
          if (hash !== expectedHash) {
            throw new Error('SillyTavern import preview is stale; run the preview again')
          }
          const created = await presets.import(source, agentPreset, content, actual, name)
          return ok(request, { agentPreset: created })
        } catch (error: unknown) {
          return err(request, presetError(agentPreset, error))
        }
      ...
      async editorOpen(request) {
        const { agentPreset } = request.payload
        const presets = ctx.get('agentPresets')
        if (presets === undefined) return err(request, noRoster(agentPreset.id as string))
        try {
          const document = await presets.openEditor(agentPreset)
      ...
      async editorSave(request) {
        const { agentPreset, composition, name, description, files } = request.payload
        const presets = ctx.get('agentPresets')
        if (presets === undefined) return err(request, noRoster(agentPreset.id as string))
        try {
          return ok(request, {
            agentPreset: await presets.save(agentPreset, {
              composition,
              ...name === undefined ? {} : { name },
      ...
      async editorListFiles(request) {
        const { agentPreset, directory, cursor, limit } = request.payload
        const presets = ctx.get('agentPresets')
        if (presets === undefined) return err(request, noRoster(agentPreset.id as string))
        try {
          return ok(request, await presets.listFiles(agentPreset, directory, cursor, limit))
        } catch (error: unknown) {
          return err(request, presetError(agentPreset.id as string, error))
        }
      },

      async editorReadFile(request) {
        const { agentPreset, path } = request.payload
        const presets = ctx.get('agentPresets')
        if (presets === undefined) return err(request, noRoster(agentPreset.id as string))
        try {
          return ok(request, await presets.readFile(agentPreset, path))
        } catch (error: unknown) {
          return err(request, presetError(agentPreset.id as string, error))
        }
      },

      async editorTestRegex(request) {
        const { agentPreset, entry, text } = request.payload
        const presets = ctx.get('agentPresets')
        if (presets === undefined) return err(request, noRoster(agentPreset.id as string))
        try {
          // Resolve the requested immutable revision before allocating a Worker.
          // The test input is browser data, but its authority is still limited to
          // a preset the loopback client can read.
          await presets.read(agentPreset)
          return ok(request, await testRegex(entry, text))
        } catch (error: unknown) {
          return err(request, presetError(agentPreset.id as string, error))
        }
      },
    ...
    skills: {
      ...
      async list(request) {
        ...
        const presenter = await presenterScopeLeaseFor(sessionId, session)
        try {
          const skills = (await skillRegistry.list({ cwd, scope: presenter.scope })).filter(isUserInvocable)
          return ok(request, {
