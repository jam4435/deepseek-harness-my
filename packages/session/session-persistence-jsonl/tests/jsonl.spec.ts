...
import SessionStore, { AgentPresetId, AgentPresetRevision, SessionId } from '@deepseek-ai/dsh-session'
...
describe('JsonlSessionPersistence: format helpers', () => {
  it('upgrades a legacy preset id before metadata listing and retains the original artifact', async () => {
    const storage = await freshRoot()
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    const fiber = await ctx.plugin(JsonlSessionPersistence, { root: storage, compression: 'none' })
    const id = SessionId('legacy-preset')
    const path = rawLogPath(storage, '/work', id)
    const legacy = `${JSON.stringify({
      type: 'session', version: 0, id, createdAt: 1, cwd: '/work', delegationDepth: 0, agentPreset: 'minimal',
    })}\n`
    await mkdir(dirname(path), { recursive: true })
    await writeFile(path, legacy)

    await expect(ctx.sessionPersistence.upgradeLegacyAgentPresetHeaders(async preset =>
      preset === 'minimal'
        ? { id: AgentPresetId('minimal'), revision: AgentPresetRevision('a'.repeat(64)) }
        : undefined,
    )).resolves.toBe(1)
    await expect(ctx.sessionPersistence.list()).resolves.toMatchObject([
      { id, agentPreset: { id: 'minimal', revision: 'a'.repeat(64) } },
    ])
    await expect(readFile(`${path}.legacy-agent-preset-v0.bak`, 'utf8')).resolves.toBe(legacy)
    await fiber.dispose()
  })
...
describe('JsonlSessionPersistence: scanLog unit', () => {
  ...
  it('round-trips the agent preset a session was composed from', () => {
    const line = toHeaderLine({
      ...
      createdAt: 1,
      delegationDepth: 0,
      agentPreset: { id: AgentPresetId('minimal'), revision: AgentPresetRevision('a'.repeat(64)) },
    })
    ...
    expect(scanLog(Buffer.from(log)).meta.agentPreset).toEqual({ id: 'minimal', revision: 'a'.repeat(64) })
  })

  it('rejects a session header whose agentPreset is not an exact reference', () => {
