...
import type { AgentPresetReference, SessionEvent, SessionId, SessionHeader, SessionPreparation } from '@deepseek-ai/dsh-session'
import {
...
/** Parse only the retired string form that the one-way header upgrader accepts. */
function legacyPresetHeader(line: string): { record: Record<string, unknown>; presetId: string } | undefined {
  try {
    const parsed: unknown = JSON.parse(line)
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) return undefined
    const record = parsed as Record<string, unknown>
    if (record.type !== 'session' || typeof record.id !== 'string' || typeof record.agentPreset !== 'string') return undefined
    return { record, presetId: record.agentPreset }
  } catch {
    return undefined
  }
}
...
export class JsonlSessionPersistence extends SessionPersistence implements PersistenceBackend<JsonlTornMarker> {
  ...
  /**
   * Rewrite legacy string preset ids before metadata listing rejects them.
   * Each replacement keeps the original compressed artifact beside the session
   * as a recovery record until a later maintenance release decides its policy.
   * @param resolveReference - maps an old floating id to one retained revision.
   * @returns number of upgraded session headers.
   */
  override async upgradeLegacyAgentPresetHeaders(
    resolveReference: (id: string) => Promise<AgentPresetReference | undefined>,
  ): Promise<number> {
    await this.ensureRootEncoding()
    let migrated = 0
    for (const project of await this.listProjectDirs()) {
      for (const directory of await this.listSessionDirs(project)) {
        const path = join(directory, `session${logSuffix(this.compression)}`)
        await this.recoverLegacyHeaderReplacement(path)
        if (!await this.exists(path)) continue
        if (await this.upgradeLegacyHeaderAt(path, resolveReference)) migrated += 1
      }
    }
    return migrated
  }

  /** Replace one retired header while retaining a recoverable original artifact. */
  private async upgradeLegacyHeaderAt(
    path: string,
    resolveReference: (id: string) => Promise<AgentPresetReference | undefined>,
  ): Promise<boolean> {
    const source = await readFile(path)
    const first = this.compression === 'zstd'
      ? await this.readLegacyZstdHeader(source)
      : this.readLegacyTextHeader(source)
    if (first === undefined) return false
    const legacy = legacyPresetHeader(first.line)
    if (legacy === undefined) return false
    const reference = await resolveReference(legacy.presetId)
    if (reference === undefined) return false
    legacy.record.agentPreset = reference
    const header = `${JSON.stringify(legacy.record)}\n`
    const replacement = await first.rebuild(header)
    await this.replaceWithLegacyBackup(path, replacement)
    return true
  }

  /** Split one raw JSONL artifact at its exact header representation. */
  private readLegacyTextHeader(source: Buffer): { line: string; rebuild: (header: string) => Promise<Buffer> } | undefined {
    const newline = source.indexOf(0x0a)
    if (newline < 0) return undefined
    return {
      line: source.subarray(0, newline).toString('utf8'),
      rebuild: async header => Buffer.concat([Buffer.from(header, 'utf8'), source.subarray(newline + 1)]),
    }
  }

  /** Split one framed artifact at its independently compressed header frame. */
  private async readLegacyZstdHeader(source: Buffer): Promise<{ line: string; rebuild: (header: string) => Promise<Buffer> } | undefined> {
    const first = scanZstdFrames(source, 1).frames[0]
    if (first === undefined) return undefined
    const plaintext = await decompressZstdFrame(source.subarray(first.start, first.end))
    assertZstdHeaderFrame(plaintext)
    return {
      line: plaintext.subarray(0, -1).toString('utf8'),
      rebuild: async header => Buffer.concat([await compressZstdFrame(header), source.subarray(first.end)]),
    }
  }

  /** Publish replacement bytes after moving the original to a durable recovery name. */
  private async replaceWithLegacyBackup(path: string, content: Buffer): Promise<void> {
    const backup = `${path}.legacy-agent-preset-v0.bak`
    if (await this.exists(backup)) {
      throw new Error(`cannot upgrade legacy preset header: recovery artifact already exists at ${backup}`)
    }
  ...
  /** Restore an original artifact left by an interrupted two-rename replacement. */
  private async recoverLegacyHeaderReplacement(path: string): Promise<void> {
    const backup = `${path}.legacy-agent-preset-v0.bak`
    if (await this.exists(path) || !await this.exists(backup)) return
    await rename(backup, path)
  }
