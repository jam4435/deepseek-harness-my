...
import type { AgentPresetReference, SessionEvent, SessionHeader, SessionId, StorageRecord } from '@deepseek-ai/dsh-session'
...
function isHeaderLine(value: unknown): value is HeaderLine {
  return (
    ...
    && ((value as { agentPreset?: unknown }).agentPreset === undefined
      || (typeof (value as { agentPreset?: { id?: unknown; revision?: unknown } }).agentPreset === 'object'
        && (value as { agentPreset: { id?: unknown } }).agentPreset.id !== undefined
        && typeof (value as { agentPreset: { id?: unknown } }).agentPreset.id === 'string'
        && typeof (value as { agentPreset: { revision?: unknown } }).agentPreset.revision === 'string'))
  )
}
