...
import type { AgentPresetReference, SessionEvent, SessionId, SessionHeader } from '@deepseek-ai/dsh-session'
...
export abstract class SessionPersistence extends Service {
  ...
  /**
   * Upgrade legacy on-disk preset ids to exact immutable references before the
   * backend exposes them as session headers. Implementations that cannot
   * rewrite their physical artifact return zero; callers may invoke this on
   * every process start without changing already upgraded records.
   * @param _resolve - resolves a legacy preset id to the revision retained now.
   * @returns number of headers upgraded during this call.
   */
  async upgradeLegacyAgentPresetHeaders(
    _resolve: (id: string) => Promise<AgentPresetReference | undefined>,
  ): Promise<number> {
    return 0
  }
