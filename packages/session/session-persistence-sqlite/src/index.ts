  ...
  private writeRow(meta: SessionHeader): void {
    this.db.prepare(`
      INSERT INTO sessions
        (id, version, created_at, cwd, parent_session, seed_length, origin, delegation_depth, agent_preset_id, agent_preset_revision, incarnation, revision)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
      ON CONFLICT(id) DO UPDATE SET
        ...
        agent_preset_revision = excluded.agent_preset_revision
    `).run(
      meta.id,
      meta.version,
      meta.createdAt,
      ...
      meta.agentPreset?.id ?? null,
      meta.agentPreset?.revision ?? null,
      randomUUID(),
    )
  }
}
