...
export interface SessionRow {
  ...
  agent_preset_revision: string | null
}
...
export function rowToMeta(row: SessionRow): SessionHeader {
  ...
  if ((row.agent_preset_id === null) !== (row.agent_preset_revision === null)) {
    throw new Error('stored session agent preset must carry both id and revision')
  }
  return {
    version: row.version,
    ...
    ...row.agent_preset_id !== null && row.agent_preset_revision !== null
      ? { agentPreset: { id: AgentPresetId(row.agent_preset_id), revision: AgentPresetRevision(row.agent_preset_revision) } }
      : {},
  }
}
