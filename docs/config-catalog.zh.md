...
来源：[`packages/host/plugin-inventory/src/index.ts:45`](../packages/host/plugin-inventory/src/index.ts)
...
<a id="deepseek-aidsh-prompt-program"></a>
...
export interface PromptProgramEntry {
  ...
  /** Relative insertion or history-depth insertion. */
  position: PromptProgramPosition
  /** History depth for `depth` entries; zero is nearest the current turn. */
  depth?: number
  /** Stable order among entries with the same position/depth. */
  order: number
  /** Independent runtime switch. */
  enabled?: boolean
}

/** Where a prompt entry is positioned relative to the current request. */
export type PromptProgramPosition = 'relative' | 'depth'
```

Source: [`packages/preset/prompt-program/src/index.ts:37`](../packages/preset/prompt-program/src/index.ts)
...
<a id="deepseek-aidsh-regex-program"></a>
...
export interface Limits {
  ...
  maxScripts?: number
  /** Maximum input characters accepted by a phase. */
  maxInputChars?: number
  /** Maximum output characters emitted by a phase. */
  maxOutputChars?: number
}
```

Source: [`packages/preset/regex-program/src/index.ts:47`](../packages/preset/regex-program/src/index.ts)
...
export interface Config {
  ...
  /** Complete model-facing tool description; deployments may describe their shell environment. */
  description?: string
}
```
...
- `@deepseek-ai/dsh-client-ui-wallpaper`（[`packages/client/ui-wallpaper/src/index.ts`](../packages/client/ui-wallpaper/src/index.ts)）
