 ...
 */
async openEditor(id: string): Promise<{ readonly id: string readonly trust: 'system' | 'user' readonly composition: string readonly revision: string readonly name?: string readonly description?: string }>
 ...
 */
async save( id: string, expectedRevision: string, draft: { composition: string; name?: string; description?: string; files?: readonly PresetFileOperation[] }, ): Promise<string>
 ...
 */
async listFiles(id: string, directory: string = '', cursor: string = '', limit: number = 200): Promise<{ entries: PresetFileEntry[]; nextCursor?: string }>
 ...
 */
async readFile(id: string, path: string): Promise<PresetFileContent>
 ...
 */
async import( sourceId: string, id: string, json: string, preview: SillyTavernImportPreview, name?: string, ): Promise<void>
...
Source: [`packages/preset/agent-presets/src/index.ts:93`](../../packages/preset/agent-presets/src/index.ts)
...
Source: [`packages/core/agent/src/runtime-types.ts:310`](../../packages/core/agent/src/runtime-types.ts)
...
<a id="agentresponse--waterfall"></a>

#### `agent/response` — waterfall

Rewrite the assembled assistant message before it is committed to the durable transcript. Consumers must delegate when they do not own a transformation; the returned message is the one model history and replay will use on the next request.

```ts cordis-catalog
/**
 * Rewrite the assembled assistant message before it is committed to the
 * durable transcript. Consumers must delegate when they do not own a
 * transformation; the returned message is the one model history and replay
 * will use on the next request.
 * @param payload.agent - the agent whose response is being committed.
 * @param payload.turn - the turn containing the response.
 * @param payload.step - the step that produced it.
 * @param payload.message - the immutable adapter-produced message.
 * @param payload.signal - the current turn cancellation signal.
 * Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): listeners receive only this agent.
 * @mode waterfall
 */
'agent/response'(this: Scoped<Agent>, payload: { agent: Agent; turn: number; step: number; message: AssistantMessage; signal: AbortSignal }, next: () => Promise<AssistantMessage>): Promise<AssistantMessage>
```
...
Source: [`packages/core/agent/src/runtime-types.ts:298`](../../packages/core/agent/src/runtime-types.ts)
